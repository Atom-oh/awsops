...
**채택됨 (2026-08-31) — 목표 아키텍처 승인, Phase 1 코드 구현됨, 프로덕션 apply 미수행.**
**Accepted (2026-08-31) — target architecture accepted, Phase 1 implemented in code, no production apply performed.**
...
|---|---|---|
| 1 | **코드 구현됨; 프로덕션 apply 미수행** / **Implemented in code; not production-applied** | Steampipe 전역 limiter, sync Lambda backpressure, 15분 비동기 이벤트 만료, 구조화 lifecycle 로그, 예약·수동 refresh의 동일한 bounded 경로 |
| 2 | **미구현·미배포** / **Not implemented or deployed** | Aurora 기반 인벤토리 MCP 도구와 AgentCore catalog에서 직접 inventory/configuration API target의 retirement |
| 3 | **미구현·미배포** / **Not implemented or deployed** | CloudTrail·cost·FinOps의 예약 Aurora cache 및 해당 라이브 target retirement |

**현재 동작은 Phase 2 catalog retirement가 배포될 때까지 직접 AgentCore inventory/configuration API 호출이다.** Phase 1은 그 호출 경로를 Aurora로 바꾸지 않는다. 이는 의도적인 현재 진실이며, 이 ADR의 목표 아키텍처를 이미 라이브인 것처럼 서술해서는 안 된다.
**Current behavior remains direct AgentCore inventory/configuration API calls until Phase 2 catalog retirement is deployed.** Phase 1 does not reroute those calls to Aurora. This is intentional current truth; the target architecture must not be described as already live.
...
- Phase 2가 배포될 때까지 AgentCore inventory/configuration 호출의 쿼터 노출은 남는다.
  (Quota exposure from AgentCore inventory/configuration calls remains until Phase 2 is deployed.)
...
Phase 1 sync Lambda는 `inventory_sync_dispatch`, `inventory_sync_complete`, `inventory_sync_busy`, `inventory_sync_failed` JSON 이벤트를 남긴다. terminal 이벤트에는 `resource_type`, `elapsed_ms`가 있고 성공에는 `row_count`, 실패에는 `error_category`와 안전하게 정리된 오류 유형이 있다.

The Phase 1 sync Lambda emits JSON events `inventory_sync_dispatch`, `inventory_sync_complete`, `inventory_sync_busy`, and `inventory_sync_failed`. Terminal events include `resource_type` and `elapsed_ms`; success includes `row_count`, while failure includes an `error_category` and safely classified error type.
