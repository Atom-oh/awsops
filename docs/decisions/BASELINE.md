...
| GATED collector scope | `INVENTORY_HOST_ONLY` runtime env | Default off; requires an expected host ID and exactly one enabled matching host row. Read-only STS verification; bounded transient retries, fatal rejection and serialized stop/restart. Deployment must prepare registry enforcement before enabling; no live-state claim. | ADR-005, ADR-011, ADR-021 |
