...
- **2026-06-03 (co-agent 3-AI review) — 문서 정정**: (1) 본문 다이어그램과 표의 **"Bedrock Opus 4.6"** 표기는 위 편차에 따라 **Sonnet 4.6**으로 읽어야 합니다(다이어그램 ASCII는 미갱신). (2) 본문이 라우터를 **"17-route classifier"** 로, References가 ADR-002를 "17-route architecture"로 인용한 것은 **오기입니다 — 정답은 11-route**(ADR-011/016/025). (3) 아래 Negative의 비용 추정($15/M, Opus 단가)은 Sonnet 전환 후 **약 $3/M**로 재계산해야 합니다.
- **2026-06-03 (co-agent 3-AI review) — doc corrections**: (1) the **"Bedrock Opus 4.6"** label in the body diagram/table should read **Sonnet 4.6** per the deviation above (the ASCII diagram was never updated). (2) The body's **"17-route classifier"** and the References citing ADR-002 as a "17-route architecture" are **incorrect — the count is 11-route** (ADR-011/016/025). (3) The cost estimate in Negative below ($15/M = Opus pricing) should be recalculated at **~$3/M** after the Sonnet switch.
