...
- S3 security drill-downs: the S3 page gains v1's Bucket Map by Region (block tiles colored Public=red > Versioned=green > Standard=cyan with an explicit Unknown=gray for buckets whose flags aren't synced — never a confident Standard; block click opens the detail panel; sample-labeled past the 500-row cap), and the bucket detail gains an 'IAM Roles with S3 Access' section (roles whose newly synced attached managed policies match AmazonS3*/AdministratorAccess, max 30; inline/bucket-policy access is explicitly out of scope, and pre-sync rows show a 'not synced' note rather than an empty all-clear — visible after a terraform apply + the next sync).
...
- S3 보안 드릴다운: S3 페이지에 v1의 리전별 버킷 맵(블록 타일 — Public=빨강 > Versioned=초록 > Standard=시안, 플래그 미동기화 버킷은 명시적 Unknown=회색으로 표시해 확정 Standard로 읽히지 않음; 블록 클릭 시 상세 패널, 500행 캡 초과 시 표본 기준 표기), 버킷 상세에 'S3 접근 권한 보유 IAM Role' 섹션(신규 sync되는 연결 관리형 정책이 AmazonS3*/AdministratorAccess에 매칭되는 role 최대 30개; 인라인/버킷 정책 경유 접근은 범위 밖임을 명시, 정책 목록 sync 전에는 빈 무혐의 대신 '미동기화' 안내 — terraform apply + 다음 sync 후 표시) 추가.
