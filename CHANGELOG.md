...
- Compliance completion email: when a benchmark run finishes, a best-effort SNS email goes out with the benchmark name, scope, total/passed/failed counts, pass rate, and a /compliance link — reusing the AI-diagnosis notification topic, flag, and admin pause switch (paused or unconfigured ⇒ silently skipped; a mail failure never affects the run).
...
- Home dashboard: a Monthly Cost Impact (est.) list — 30-day resource-count change × a static per-type unit-cost heuristic, sorted by |impact| (top 8), explicitly labeled as a heuristic rather than billing data; types with no 30-day baseline or no weight entry are excluded rather than shown as $0.
...
- 컴플라이언스 완료 이메일: 벤치마크 실행 완료 시 벤치마크명·scope·전체/통과/실패 건수·통과율·/compliance 링크가 담긴 best-effort SNS 이메일 발송 — AI 진단 알림의 토픽·플래그·관리자 일시중지 스위치를 재사용(중지/미설정이면 조용히 생략, 메일 실패는 실행 결과에 영향 없음).
...
- 홈 대시보드: '월 비용 영향 추정' 리스트 — 30일 리소스 수량 변화 × 타입별 정적 단가 휴리스틱, |영향| 내림차순 상위 8, 청구 데이터가 아닌 근사임을 명시; 30일 기준값이 없거나 단가 항목이 없는 타입은 $0로 표시하지 않고 제외.
