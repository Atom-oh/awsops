...
- PR review pipeline: all 12 model/lens reports must include a body and a unique final completion marker; Kiro tool chatter cannot supply the body after tool use, while its numeric usage footer remains accepted. Failed/timed-out CLI output is discarded, chair and scrubber exits are checked, and bounded retries/hard-kill backstops fit a 90-minute workflow ceiling with a 900s chair timeout. Pod Identity preflight checks the existing provider and signed caller identity before each model phase without exporting credentials, writing credential files, forcing renewal or imposing a cached-lease TTL floor; ambient SDK refresh remains available. Labeled session tokens are scrubbed, and offline regressions run through the documented full test suite. Code-first three-pass diff ordering (code → decisions/runbooks → docs prose) and the sanitized, narrowly-scoped truncated-file guard continue to downgrade unverifiable absence-claims to MINOR instead of dropping them.
...
- PR 리뷰 파이프라인: 12개 모델/lens 보고서 모두 본문과 유일한 마지막 완료 마커가 필수이며, Kiro 도구 사용 뒤의 도구 출력은 본문으로 세지 않되 숫자 사용량 footer는 허용한다. 비정상 종료·타임아웃 CLI 출력은 폐기하고 chair와 scrubber 종료 상태를 확인하며, 제한된 재시도·하드킬 백스톱과 900s chair 타임아웃을 90분 워크플로 상한에 맞춘다. 각 모델 단계 전 Pod Identity preflight가 기존 provider와 서명된 호출자 신원을 확인하되 자격 증명 export·파일 저장·갱신 강제·캐시 lease 잔여 시간 하한 없이 SDK 자동 갱신 경로를 유지한다. 이름이 붙은 세션 토큰을 마스킹하고 오프라인 회귀 테스트를 문서화된 전체 테스트에 포함한다. 코드 우선 3-pass diff 정렬(코드 → decisions/runbooks → docs 산문)과 새니타이즈된 좁은 범위의 절단 파일 가드(검증 불가 부재 주장을 삭제 대신 MINOR로 하향)는 유지한다.
