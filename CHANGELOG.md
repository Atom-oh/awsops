...
- PR review pipeline: all 12 model/lens reports require a unique final single-line JSON frame bound to a fresh per-cell nonce; only the validated, decoded and scrubbed report reaches the chair. Frame counting uses the expected cell nonce, so earlier other-nonce source examples remain uncredited chatter while current-nonce duplicates and trailing output fail closed. Tool strings, Markdown fences and static marker examples inside the report remain data, with no completion heuristic or legacy-marker fallback; Kiro assistant prefixes and one numeric usage footer are cosmetic. Rejected previews hide encoded frame payloads and retain bounded scrubbed diagnostics. Failed/timed-out CLI output is discarded, chair and scrubber exits are checked, and each Codex/Kiro lens has up to two 1200s attempts with hard-kill backstops within a 90-minute workflow ceiling and a 900s chair timeout. Pod Identity preflight checks the existing provider and signed caller identity before each model phase; ambient SDK refresh remains available. Labeled session tokens are scrubbed, and offline regressions run in CI and the local full test suite. Code-first three-pass diff ordering (code → decisions/runbooks → docs prose) and the sanitized, narrowly-scoped truncated-file guard continue to downgrade unverifiable absence-claims to MINOR instead of dropping them.

## [0.9.0] - 2026-08-22

**Upstream release history:** this date and version belong to the [published `whchoi98/awsops` v2 release](https://github.com/whchoi98/awsops/releases/tag/v0.9.0), tagged at `f22be3a08e1af902126f1a857959ec36d4e91fb3`. The source is integrated here later; local integration fixes are tracked above under `[Unreleased]`. The date is retained from upstream.
...
- 토폴로지 서비스·네트워크 보기(`/topology?view=e2e`): 구성된 진입 경로, 저장된 서비스 관측, Network Flow Monitor 트래픽을 범위가 확인된 엔드포인트 식별자로 연결한다. 모니터·메트릭·기간을 명시적으로 조회해 전송량·RTT·재전송·타임아웃·NAT 주소·경유 구성요소를 확인하며, 구성·서비스·네트워크·식별자·문맥 관계를 구분한다. 검색과 주요 흐름 확대, 수집 시각·부분 결과 안내를 제공하고 하나의 추적된 요청이나 순서가 확인된 패킷 경로로 단정하지 않는다. 관측 결합은 호스트·기본 리전 범위이며 계정 전환 시 이전 결과를 버리고 EKS 사설 IP는 리전·VPC 범위로 식별한다. 근거 필터는 검색과 상세에도 적용하며 화면 한도는 관측 주변을 우선한다. 인벤토리 조회 실패와 수집 실패·부분 수집·실행 중 상태를 구분하면서 저장된 행을 유지한다. 구성 시각은 반환된 행의 수집 범위와 타입별 마지막 성공 시각으로 표시하고 self 이외 범위에는 host run을 적용하지 않는다. 수집 메타데이터가 있으면 부분 수집·오래된 데이터·이전 스냅샷 유지와 실패 이유를 표시하며, 스냅샷만 제공하는 producer의 메타데이터 부재는 중립적인 수집 상태 미확인으로 처리한다. 기존 Network Flow와 서비스 맵에서 바로 이동할 수 있다.
...
### Fixed
- 토폴로지 IP 타겟은 리전/VPC와 서브넷 또는 Pod 근거가 일치할 때만 서비스에 연결하며, 호스트 EKS 소유 정보를 멤버 계정 그래프에 섞지 않습니다. 종료된 Pod가 재사용된 IP의 현재 소유자를 가리지 않도록 제외하고, 그룹화한 IPv6 타겟의 주소·포트 표기를 명확히 했습니다. NFM 조회 응답은 캐시된 원래 관측 구간과 상위 기여자 조회 상한 정보를 제공합니다.
...
- 트레이스 서비스 맵: 서비스 식별자는 데이터소스·계정·리전·환경·네임스페이스·클러스터별로 분리하고 비동기 span link와 식별 가능한 브로커/큐 관계를 보존한다. 동일한 큐 ARN은 같은 데이터소스·환경 안에서 호출자의 계정·리전을 넘어 연결하지만, writer·API·SQL/AI 읽기는 보존된 행도 destination ARN의 한정자에서만 계정·리전 claim을 재계산한다. 비-ARN·잘못된 ARN·누락 한정자는 null이며 호출자·레거시 폴백은 없다. 화면은 claim 값과 미검증 텔레메트리 고지를 함께 표시하며 AWS 큐 소유권이나 인벤토리 연결을 증명하지 않는다. DB 호스트 매칭에는 기존 계정 부재·`self` 분기에 신뢰된 설정 `HOST_ACCOUNT_ID`와 명시적 계정이 일치하는 새 분기를 추가한다. Tempo의 선행 0이 생략된/64비트 hex ID를 전체 OTLP 바이트와 연결하고, 0 부모 ID는 부재로 처리하되 0 trace/span ID는 거부한다. 실패·부분 수집·정상 빈 결과·오래된 데이터를 API·화면·AI 읽기 뷰에 표시하고, 수집 실패 시 이전 그래프를 보존하며 호출량을 신뢰도 확률로 표현하지 않는다. inventory-reader Lambda도 웹과 같은 `graph_rebuild_interval_mins` 설정을 전달받으며, 0은 기존 15분 freshness 하한을 유지한다. projection 마이그레이션(`make migrate`)과 기존 Terraform 운영 절차를 통한 `inventory_read_mcp` 재배포가 필요하며 [적용·식별 계약](docs/runbooks/source-sync-observability.md)을 참고한다.
- AI 진단 근거: 누락·실패·부분 관측을 정상 또는 개선으로 판정하지 않는다. 유효한 0·관측된 위반 테스트는 정규화된 평가기 계약을 검증한다. 현재 진단 수집기는 X-Ray `to_ref`만 제공하고 `to`와 `inventory.unencrypted` 집계는 제공하지 않으므로, 필요한 근거를 연결하기 전까지 운영 edge·암호화 불변식은 `unknown`으로 남는다. 누락된 incident 신뢰도는 보수적으로 처리하며 PDF 렌더러의 외부 리소스 요청을 차단한다.
- FinOps rightsizing MCP: Compute Optimizer 응답의 자원별 권장·절감액 필드를 실제 SDK 계약에 맞추고 미확인 절감액과 실제 0을 구분한다. 조회 누락·오류·통화 불일치가 있으면 완전한 합계를 만들지 않는다.
...
- PR 리뷰 파이프라인: 12개 모델/lens 보고서 모두 셀마다 새 nonce로 결합한 유일한 마지막 한 줄 JSON 프레임이 필수이며, 검증·디코딩·마스킹한 본문만 chair로 전달한다. 현재 셀의 nonce를 가진 프레임만 세어 앞선 다른 nonce의 소스 예문은 인정하지 않는 도구 출력으로 처리하며 현재 nonce 중복과 뒤따르는 출력은 거부한다. 본문의 도구 문자열·Markdown fence·정적 마커 예문은 데이터로 유지하고 완료 추정이나 레거시 마커 폴백은 사용하지 않는다. Kiro assistant 접두사와 숫자 footer 하나는 표시 형식으로만 허용하며 거부된 프레임 payload는 preview에서 숨기고 나머지 진단은 마스킹 후 제한한다. 비정상 종료·타임아웃 CLI 출력은 폐기하고 chair와 scrubber 종료 상태를 확인하며, Codex/Kiro 각 렌즈에 최대 1200초씩 두 번의 시도를 허용하고 하드킬 백스톱과 900s chair 타임아웃을 90분 워크플로 상한에 맞춘다. 각 모델 단계 전 Pod Identity preflight가 기존 provider와 서명된 호출자 신원을 확인하며 기존 SDK 자동 갱신 경로를 유지한다. 이름이 붙은 세션 토큰을 마스킹하고 오프라인 회귀 테스트를 CI와 로컬 전체 테스트에 포함한다. 코드 우선 3-pass diff 정렬(코드 → decisions/runbooks → docs 산문)과 새니타이즈된 좁은 범위의 절단 파일 가드(검증 불가 부재 주장을 삭제 대신 MINOR로 하향)는 유지한다.

## [0.9.0] - 2026-08-22

**Upstream 릴리스 이력:** 이 날짜와 버전은 커밋 `f22be3a08e1af902126f1a857959ec36d4e91fb3`에 태그된 [공개 `whchoi98/awsops` v2 릴리스](https://github.com/whchoi98/awsops/releases/tag/v0.9.0)의 이력이다. 이후 이 저장소로 통합된 소스이며 로컬 통합 수정은 위 `[Unreleased]`에서 추적한다. 날짜는 upstream 원본을 보존한다.
