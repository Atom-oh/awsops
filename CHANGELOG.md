...
- Prometheus/Mimir AI 쿼리 생성은 이제 정확한 인스턴스에 기반해 닫힌 방식으로 동작: 한국어 리소스 개념으로 관련 메트릭을 우선하고, 최대 2개 메트릭 메타데이터 후보를 업스트림 3초 제한으로 조회하며, 생성 PromQL은 캐시 또는 커넥터가 존재를 확인한 메트릭만 참조해야 함. 후보는 read-only 5초 제한으로 1회 실행해 문법/타입을 검증하고 확정 오류는 AI 교정 1회만 허용; grounding 부재나 일시적 커넥터 실패는 503. PromQL 생성은 인스턴스 id가 필수이며 다른 레거시 slug DSL 경로는 유지.
