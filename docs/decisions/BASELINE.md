...
> 2-티어: **FROZEN**(do-not-enable, 풀려면 새 ADR+패널+owner-override) vs **GATED**(거버넌스 하 활성화 가능, 현재 OFF). 아래 **feature gate** 는 전부 terraform flag default=false 와 일치한다. 예외로 마지막 행의 **마이그레이션 창 스위치**는 기본 **true** 다 — 기능을 켜는 플래그가 아니라 이관이 끝날 때까지 legacy 동작을 유지하는 스위치이므로, 기본값이 반대다.
