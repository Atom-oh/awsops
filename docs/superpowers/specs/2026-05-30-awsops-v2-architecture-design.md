...
```
CloudFront → ALB → ┌─ awsops-web (Fargate, 경량: 라우팅·SSE만)
                   └─ awsops-steampipe (Fargate, FDW PostgreSQL)
                          │ enqueue
                          ▼
...
1. **명령 최소화**: `make bootstrap` → `terraform apply` → `make deploy ENV=dev`. 17단계 → 3명령.
...
**스택 분할**(실패 도메인 기준, ADR-024 정신 계승): `network` / `data`(Aurora) / `workload`(ECS·web·steampipe) / `ai`(AgentCore·워커·SQS·SFN) / `incident`.
...
|---|---|---|---|
| **P1** | Terraform 기반 인프라 + 배포 토폴로지 | VPC/ECS/Aurora/ECR/ALB/CloudFront/Cognito 스택, CI/CD 파이프라인, `make` 래퍼, AgentCore 멱등 provisioner, 빈 web 띄우기 | `terraform apply`로 신규 도메인에 web 헬스체크 통과 |
