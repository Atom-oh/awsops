...
## Role
105 client components across 14 subdirectories: `ui` (shared primitives), `shell` (AppShell, Sidebar, LanguageProvider, AccountSelector, etc.), `charts`, `chat`, `inventory` (+`metrics/`), `eks`, `diagnosis`, `datasources`, `finops`, `graph`, `insights`, `nfm`, `overview`, `topology`.
