...
| AI | AgentCore Runtime, section gateways, Lambda MCP tools, Memory and Code Interpreter; models/catalog are configuration-driven | Live read-only domain tools. The registered BFF-local `aws-data` and collector routes are hard-disabled by `steampipeAvailable()` and fall back to normal routing. SSM is the runtime configuration source. | `ai.tf`, `scripts/v2/agentcore/`, `agent/`, `web/lib/aws-data.ts` |
