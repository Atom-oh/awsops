...
> v1.8.0(`src/`, CDK/EC2/Steampipe, `/awsops` basePath)은 **ADR-016(2026-07-09)에 따라 단계적 폐기 진행 중**입니다 — `docs/runbooks/v1-decommission.md` 참조. 폐기 완료 전까지 v1 규칙(특히 `/awsops` fetch 접두사, Steampipe pg Pool)은 **v2에 적용되지 않습니다.** v1 세부는 `src/**/CLAUDE.md` 참조.
...
> v1.8.0 (`src/`, CDK/EC2/Steampipe, `/awsops` basePath) is **being decommissioned in stages per ADR-016 (2026-07-09)** — see `docs/runbooks/v1-decommission.md`. Until decommission completes, v1 rules (notably the `/awsops` fetch prefix and Steampipe pg Pool) **do NOT apply to v2**. For v1 detail see `src/**/CLAUDE.md`.
