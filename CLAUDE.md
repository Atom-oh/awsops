...
## ADR
`docs/decisions/ADR-*.md` (001–036). v2 관련: **030**(ECS Fargate + Aurora) 채택; **029·031·032·033·034·035·036** v2 신규 ADR **전부 멀티AI 합의 리뷰로 Accepted (2026-06-09)** (029는 REVISE 반영 개정 — 메커니즘은 036 하이브리드로 위임·6대 통제 유지; 031/032/034/036은 ACCEPT-WITH-CHANGES). **009는 032로 대체됨**(상관분석 엔진은 032 Triage로 보존). **Proposed 잔여 없음.** 신규 ADR 번호 = 최고번호+1. ADR 인덱스/정정 노트는 `docs/decisions/CLAUDE.md`.
...
## ADR
`docs/decisions/ADR-*.md` (001–036). v2-relevant: **030** (ECS Fargate + Aurora) adopted; **029·031·032·033·034·035·036** — all v2 ADRs — are now **Accepted (2026-06-09) via multi-AI consensus review** (029 revised from a REVISE verdict — mechanism deferred to 036's hybrid, six controls retained; 031/032/034/036 ACCEPT-WITH-CHANGES). **ADR-009 is superseded by 032** (its correlation engine carried into 032's Triage). **No Proposed ADRs remain.** New ADR number = highest + 1. ADR index/correction notes: `docs/decisions/CLAUDE.md`.
