...
- Add a first-class **Integrations axis** (Capability Providers) covering observability (Grafana/Datadog/Splunk/New Relic/Prometheus, READ) **and** read/write knowledge/work systems (Notion/Confluence/Jira/ServiceNow/Slack, READ_WRITE) — writes governed by the existing mutating-action gate.
...
**Object model — `integrations` (new table):**
- `id`, `name`, `kind` (`grafana | datadog | splunk | prometheus | newrelic | notion | confluence | jira | servicenow | slack | github | gitlab | custom_mcp`), `description`.
- `connection`: endpoint URL; `transport` (`sigv4 | oauth_client_credentials | oauth_3lo | api_key`); `credentials_ref` (Secrets Manager ARN — never plaintext); optional `private_connection_ref` (VPC path).
- `capability`: **`READ | READ_WRITE`** — the load-bearing governance attribute.
- `exposed_tools JSONB` (allowlistable tool ids), `provided_context JSONB` (typed context schema: topology/dashboards/wiki — injected into the effective spec).
...
- **Integrations / Capability Providers**: register (kind picker → connection → auth wizard: SigV4 / OAuth / API-key → review), per-vendor onboarding presets (Notion/Confluence/Grafana/Datadog), tool allowlisting per Agent Space, READ vs READ_WRITE, health check. Credentials → Secrets Manager.
- **MCP servers** (custom_mcp kind): generic BYO-MCP registration.
...
- **P2 — Integrations axis (READ):** `integrations` registration UX (SigV4-first) + per-space tool allowlisting + read context injection; first observability integrations (Grafana/Datadog) as read MCP; resolver context injection.
