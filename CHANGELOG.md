...
- Topology infra page gains two columnar map views — a 5-column infra resource map (External | VPC | Subnet | Compute | NAT) and a per-cluster K8s map (Ingress → Service → Pod → Node) — rendered as a real graph (fixed-column ReactFlow with edge lines), with click cross-highlighting, search highlighting, and a color legend. Built entirely on existing inventory/EKS reads; the only server-side addition is the read-only `ingresses` in-cluster kind.
...
- 토폴로지 인프라 페이지에 컬럼형 맵 뷰 2종 추가 — 5컬럼 인프라 리소스 맵(External | VPC | Subnet | Compute | NAT)과 클러스터별 K8s 맵(Ingress → Service → Pod → Node) — 고정 컬럼 ReactFlow에 실제 엣지 연결선을 그리는 그래프로 렌더링되며, 클릭 교차 하이라이트·검색 하이라이트·색상 범례 포함. 전부 기존 인벤토리/EKS 조회만 사용하고, 서버 측 추가는 read-only `ingresses` in-cluster kind 1종뿐.
