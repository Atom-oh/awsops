...
- FinOps baseline-recommendations engine (ADR-019, extends ADR-012, `finops_baseline_enabled`): a daily Fargate batch evaluates a rule catalog (unattached EBS volumes; EC2/RDS rightsizing via Compute Optimizer) against Cost Explorer/`inventory_resources`/Compute Optimizer/Cost Optimization Hub — no CUR/Athena in this repo, so amounts come only from a published rate card or an API's own estimate, never invented. The deterministic engine owns status/amount/priority; an LLM adds a short Korean explanation only, discarded if it states a different number. False-positive guards (protected tags, insufficient Compute Optimizer observation window) demote to `needs_review` rather than hiding a finding. Read-only — no new AWS-mutation path. New `/cost` section (`GET /api/finops/findings`); fixes the ADR-012/terraform drift where `cost-optimization-hub:*` was documented but never granted (the FinOps MCP's Cost Optimization Hub tool has been `AccessDenied` since ADR-012).
...
- FinOps 기본 권장 엔진 추가(ADR-019, ADR-012 확장, `finops_baseline_enabled`): 일별 Fargate 배치가 룰 카탈로그(미사용 EBS 볼륨; Compute Optimizer 기반 EC2/RDS rightsizing)를 Cost Explorer/`inventory_resources`/Compute Optimizer/Cost Optimization Hub에 평가 — 이 저장소엔 CUR/Athena가 없어 금액은 공개 요율표 또는 API 자체 추정치로만 산출되며 절대 발명되지 않음. 결정론적 엔진이 판정·금액·우선순위를 소유하고, LLM은 한국어 설명만 덧붙이며 확정 숫자와 다른 금액을 말하면 폐기됨. 오탐 가드(보호 태그, Compute Optimizer 관측 기간 부족)는 항목을 숨기지 않고 `needs_review`로 강등. read-only — 신규 AWS-변경 경로 없음. `/cost`에 새 섹션 추가(`GET /api/finops/findings`); ADR-012/terraform 드리프트 수정(`cost-optimization-hub:*`가 문서화됐지만 실제로 부여된 적이 없어 FinOps MCP의 Cost Optimization Hub 툴이 ADR-012 이후 상시 `AccessDenied`였음).
