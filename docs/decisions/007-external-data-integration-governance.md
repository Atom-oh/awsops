...
> **개정 (2026-07-31, ADR-017 PR #194):** §5("큐레이션 커넥터만")와 §6("모든 write payload는 server-side DLP/redaction·human-gate 통과")는 `mcp.lambda` target(자체 Lambda 핸들러)을 전제로 서술됐다. ADR-017이 도입한 `mcpServer`-type target(원격 벤더 MCP 프리셋)은 **같은 기술적 공백**을 갖는다 — provision.py에 `tools/list` 동등 control-plane API가 없어 벤더가 실제로 광고하는 툴 목록을 검증할 수 없고, 따라서 서버측 DLP/redaction/human-gate를 그 원격 target 앞에 강제로 끼워넣을 지점이 없다. 이는 §5의 "큐레이션 = 기술적으로 강제된 provenance 보증"이라는 프레임과, §6의 DLP/human-gate가 "모든 외부 write에" 적용된다는 프레임 양쪽에 대한 **문서화되고 스코프가 좁혀진 예외**다 — 여기서 "큐레이션"은 여전히 유효하지만 그 의미는 `preset_key`(admin이 사전 등록한 벤더 한정) + `official_mcp_read_only_ack`(운영자 attestation)이며, 기술적 provenance 보증이 아니다. 상쇄: capability는 항상 `read`로 고정(이 예외는 write 경로를 열지 않음), `official_mcp_enabled`는 기본 false, `integrations_write_enabled`(§6 write 게이트)는 이 기능과 독립적으로 여전히 기본 false다 — 따라서 §6의 DLP/human-gate 요구사항 자체는 (아직 켜지지 않은) 광역 write 티어에 계속 적용되며, 이 예외로 인해 그 요구사항이 약화되는 것은 아니다. 상세는 ADR-017 §Trade-offs·§Security, ADR-004 위 개정(같은 기술적 공백에 대한 대응 사항) 참조.
>
> **Amendment (2026-07-31, ADR-017 PR #194):** §5 ("curated connectors only") and §6 ("every write payload passes server-side DLP/redaction + human-gate") were both written assuming `mcp.lambda` targets (our own Lambda handler). The `mcpServer`-type targets (remote vendor MCP presets) ADR-017 introduces have the **same technical gap**: provision.py has no `tools/list`-equivalent control-plane API to verify what the vendor's server actually advertises, so there is no point at which server-side DLP/redaction/human-gate could be inserted in front of that remote target. This is a documented, narrowly-scoped exception to both §5's framing of curation as a technically-enforced provenance guarantee (curation here still holds, but means `preset_key` + `official_mcp_read_only_ack` operator attestation, not a technical provenance guarantee) and to §6's DLP/human-gate applying to "every" external write. Offsetting factors: capability is always fixed to `read` (this exception never opens a write path), `official_mcp_enabled` defaults to false, and `integrations_write_enabled` (§6's write gate) remains independently default-false — so §6's DLP/human-gate requirement itself still applies in full to the (not-yet-enabled) broad write tier; this exception does not weaken that requirement. See ADR-017 §Trade-offs/§Security and the ADR-004 amendment above (addressing the same technical gap from the tool-allowlist angle) for detail.
