...
3. **Full ADR-039 §7 mutating-gate controls.** `action_catalog` facade (`executor_type='lambda'`), **`enabled=false` default**, mandatory **dry-run**, **4-eyes (approver≠creator)** or logged single-operator escape (ADR-029 §4), **paired rollback_ref**, **kill-switch (fail-closed)**, idempotency token (ADR-036 `job_id==SFN execution-name`), audit (Aurora + S3 Object-Lock). Model only *proposes inputs* — never writes directly.
   ...
   **Control-semantics mapping for non-AWS comms writes (2026-06-16 amendment).** The six controls were designed for **idempotent, reversible AWS-resource operations**; a Slack message / Jira ticket / Notion page is neither. The controls are therefore **re-mapped, not applied literally** (the panel flagged that taking them verbatim breaks their meaning):
   - **dry-run → mandatory draft-render.** No SaaS "dry-run" API exists; the gate renders the exact payload (message/ticket/page body) post-DLP for human review. This *is* ADR-040 Option 4 promoted to a required pre-step, not an optional fallback.
   - **paired rollback_ref → compensating action + audit, best-effort.** A sent message can be `chat.delete`d, a ticket transitioned/closed, a page archived — but these are **compensating actions, not state restoration**, and an already-read notification is irreversible. The `rollback_ref` records the compensating handle + an immutable audit entry; **it must not be presented as a true undo.** Where no compensating action exists → the connector is **draft-only** (Option 4).
   - **idempotency token → kept as-is (best fit).** `job_id`-keyed dedup genuinely prevents double-posting; native idempotency keys are used where the SaaS supports them (e.g. Jira), client-side dedup where it does not (e.g. Slack).
   - **4-eyes + kill-switch + DLP/redaction + audit → kept literally** (these transfer cleanly and carry the exfiltration defense, Condition 2).
