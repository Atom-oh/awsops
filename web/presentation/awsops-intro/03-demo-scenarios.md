...
<div class="ds">
  ...
  <div class="ds-btns">
    ...
    <button class="db db2" id="dDl2" disabled>&#x2193; PPTX</button>
...
(function(){
  ...
  function tick(){if(run)tm.textContent=fmt(Math.floor((Date.now()-t0)/1000))}
  function next(){
    if(idx<S.length){if(idx>0)setI(idx-1,'\u2705');setI(idx,'\u23F3');prog();idx++;to=setTimeout(next,800)}
    else{setI(S.length-1,'\u2705');prog();run=false;clearInterval(iv);bD.disabled=false}
  }
  ...
  bD.addEventListener('click',function(){alert('PPTX download is a simulation.')});
})();
</script>
:::
