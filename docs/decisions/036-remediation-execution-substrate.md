...
> **⛔ REVERSED (AWS-resource scope) / PARTIAL-UNFREEZE (external DATA, via ADR-040)** — owner decision via 3-AI consensus (kiro/codex/gemini; see `docs/reviews/2026-06-11-high-risk-adr-reversal-consensus.md`). The **AWS-resource** remediation/mutation **execution substrate is abandoned** with ADR-029 — AWSops does not become a write control plane for AWS resources; mutation stays with the operator's SSM/Change Manager/IaC/console. Built but shipped flag-OFF and now **frozen** (`remediation_enabled=false`, **do-not-enable**); dark code kept, not deleted.
>
> **Scope clarification (2026-06-16):** "frozen" applies to the **AWS-resource** scope (the SSM Automation + Change Manager executor branch). Per **ADR-040/041**, the **P2 `lambda` executor branch** of this same spine may be **reused for non-AWS-resource external DATA writes** (Slack/Notion/Jira) under a **separate gating layer** (own `integrations_write_enabled` flag + dedicated kill-switch + no-AWS-mutation IAM). This is **the shared P2 worker SFN — not a second engine** (matches the companion spec §7 "single spine"); ADR-040's "two fully-independent control planes" refers to the **gating/IAM layer**, not the orchestration engine. AWS-resource SSM/Change-Manager automation stays frozen. "Accepted" record below retained as history. / "동결"은 **AWS-리소스 스코프**(SSM/Change Manager 실행기 분기)에 한정; ADR-040/041에 따라 동일 spine의 **P2 lambda 실행기 분기**는 비-AWS 외부 DATA write에 별도 게이팅 층으로 재사용 가능 — **공유 P2 SFN이며 별도 엔진 아님**(ADR-040의 "두 독립 컨트롤 플레인"은 게이팅/IAM 층을 가리킴).
