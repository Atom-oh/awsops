...
- 비용 페이지 퀵윈: 일평균·전월 총액 KPI 타일 + 서비스 타일의 'N개 >20% 증가' 서브텍스트, 로드는 성공했지만 데이터가 0건일 때 Cost Explorer 온보딩 배너(Billing 콘솔에서 활성화 — 표시까지 최대 24시간), 서비스 테이블의 임계값 색상 변화율 셀(>20% red, >0 orange, <0 green; 기준월 없음은 '—')과 점유율 미니 바 + 실제 숫자 정렬.
