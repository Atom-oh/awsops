...
### Negative / Trade-offs
- **원격 target에는 우리 쪽 서버측 toolAllowlist 강제가 없다(2026-07-31 kiro 리뷰로 정정).** Lambda target(`mcp.lambda`)은 `toolSchema.inlinePayload`가 노출 툴 집합을 하드 리밋하지만, `mcpServer` target은 `listingMode=DEFAULT`로 벤더 서버가 광고하는 툴 전부(쓰기 툴 포함 가능)를 그대로 노출한다 — control-plane API에는 `tools/list` 동등 기능이 없어 provision.py가 벤더의 실제 툴 목록을 검증할 수 없다. `read_only_note`(catalog.py)는 여전히 "벤더측 설정(RBAC 스코프/`--disable-write`/preview 등)을 신뢰"하는 텍스트 메모일 뿐이다. **완화**: `terraform.tfvars`의 `official_mcp_read_only_ack[preset_key]`에 운영자가 `read_only_note`를 실제로 검증한 **엔드포인트 URL 문자열 그대로**(불리언 `true`가 아니다 — 변수 타입은 `map(string)`이며 provision.py는 `ack[preset_key] == official_mcp_endpoints[preset_key]`인지 비교한다) 기록해야 하며, 비어있거나 현재 엔드포인트와 다르면 provision.py가 해당 프리셋을 fail-closed로 SKIP(및 기존 target 회수)한다 — 이것은 벤더 측 컨트롤을 provision.py가 대신 검증해주는 것이 아니라, "운영자가 이 엔드포인트를 검증했다"는 사실을 기록하는 게이트다. 예:
  ```hcl
  ...
  ```
- **ack는 "툴 목록"이 아니라 "벤더측 컨트롤"에 대한 attestation이다 — 벤더가 나중에 추가한 툴은 재-ack 없이 흡수된다(round-4 리뷰 MAJOR L3-1, 2026-07-31 명시화).** `listingMode=DEFAULT` + provision.py가 매 실행 `synchronize_gateway_targets`를 호출(EXISTS 경로 포함, 벤더의 툴 변경을 반영하기 위한 round-3 수정)하므로, 벤더가 새 툴(잠재적으로 쓰기 툴)을 배포하면 다음 `make agentcore`에서 게이트웨이에 그대로 등록된다. **툴 집합 단위로 게이트할 수 없다**: `bedrock-agentcore-control`에는 툴 목록을 읽는 오퍼레이션이 아예 없고(target 관련 오퍼레이션은 `Create/Get/List/Update/DeleteGatewayTarget` + `SynchronizeGatewayTargets`뿐이며 응답에 툴 이름이 없음 — botocore `2023-06-05` 모델 확인), 툴 집합을 고정할 수 있는 유일한 필드 `McpServerTargetConfiguration.mcpToolSchema`는 AWS 문서상 **authorization code grant 자격증명에서만 지원**되고(이 프리셋들은 API key) 설정 시 툴 동기화 자체가 비활성화된다. 따라서 **잔여 리스크의 실질적 방어선은 벤더측 컨트롤(RBAC 스코프 / `--disable-write` / read-scoped 토큰)** 하나뿐이며 — 이것은 나중에 추가되는 툴에도 계속 적용된다 — ack는 그 컨트롤을 확인했다는 기록이다. 운영 요구사항: 프리셋을 켠 뒤에도 **벤더의 릴리스 노트/툴 목록 변경을 주기적으로 확인**하고, 스코프가 넓어졌다면 ack를 회수(해당 키 삭제 → 다음 실행에서 자동 retire)한다.
...
- **Security**: capability=read 고정. **서버측 toolAllowlist 강제는 mcpServer target에는 없다** (위 Trade-offs 참조) — 대신 `official_mcp_read_only_ack`(`map(string)`, 값 = 검토한 엔드포인트 URL 그대로. 기본 `{}` = 미승인 프리셋은 provisioning 거부)로 "운영자가 이 엔드포인트의 벤더측 read-only 컨트롤을 확인했다"는 명시적 acknowledgment를 요구하는 fail-closed 게이트로 대체한다(엔드포인트가 바뀌면 ack 불일치 → 자동 retire). `official_mcp_endpoints`는 `https://`만 허용(Terraform variable validation + provision.py 런타임 재검증, 메타데이터/루프백/링크로컬 호스트 차단)하여 토큰 평문전송·SSRF-adjacent 임의 엔드포인트를 막는다. 자격증명은 기존 Secrets Manager 시크릿을 재사용하되 `mcp:<preset_key>` 네임스페이스 키에 저장(같은 슬러그를 쓰는 datasource-connector kind-mirror와 충돌 방지). `custom_mcp` 폐기 유지로 임의 엔드포인트 attack surface 확장 없음.
