...
- **EKS onboarding**: multi-select in `configure.mjs` → `eks.tf` grants the web task role an **Access Entry + AmazonEKSAdminViewPolicy** (cluster-scoped). Automatic kubeconfig registration/lookup UI is P3. **`onboard_eks_clusters` (tfvars) covers only the Terraform path — a separate out-of-band path also exists**: if an operator runs `create-access-entry` + `associate-access-policy` (view-only) directly via CLI on another cluster, the CloudTrail-driven read-only Lambda gated by `eks_auto_register_enabled` (live: true) (`awsops-v2-eks-auto-register`, makes no AWS resource changes) observes that access entry and reflects it into Aurora's `eks_registrations` (the BFF's allow-list). In other words, **the number of clusters with a live Access Entry can exceed the length of `onboard_eks_clusters`** — verify the facts via `aws eks list-access-entries` + the `eks_registrations` table.
...
| P1c | Aurora Serverless v2 (schema based on ADR-001[legacy 030] — baseline v9 frozen, see `data/schema.sql` for table count) | ✅ |
...
- `data.tf` + `data/schema.sql` — Aurora Serverless v2 + schema based on ADR-001[legacy 030] (baseline v9 frozen — see `data/schema.sql` for table count)
...
- `app/{network-flow,dns-query,ip-addresses,vpc-endpoints,direct-connect}/` + `lib/{nfm,dns-logs,ip-inventory,vpce,dx}.ts` — the 5 network menu items: live NFM top-contributor queries + E2E hop path · Resolver/CoreDNS query-log aggregation via Logs Insights · ENI-based IP inventory (detects unused EIPs/available ENIs) · VPC endpoint idle/policy/Gateway-coverage analysis · Direct Connect down-detection/peak-utilization/BGP route visibility.
...
- `lib/aws-data.ts` + `lib/collectors/` — chat BFF-local handlers (not via AgentCore): aws-data (runs LLM-generated Steampipe SQL live) + the 6 auto-collect collectors registry — `app/api/chat/route.ts` branches to these locally.
...
Full overview: [docs/architecture.md](docs/architecture.md) (bilingual + mermaid) · New joiners: [docs/onboarding.md](docs/onboarding.md) · Full API index (84 routes): [docs/api-reference.md](docs/api-reference.md) · Operations: [docs/runbooks/](docs/runbooks/).
