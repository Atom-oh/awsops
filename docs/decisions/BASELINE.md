...
| **GATED** | diag-signal LLM 폴백 (Explore 칩) | `diag_signal_querygen_enabled` | `datasource_diagnosis_enabled` 선행. **위 플래그와 분리**되어 있다 — "ClickHouse 그래프 쿼리 1건"에 대한 동의가 "fallback 대상 kind 전체에 매일 LLM 생성 + 라이브 dry-run"에 대한 동의는 아니다(PR #205 리뷰 MAJOR, 3개 렌즈 4모델). 결정론 매처가 **ready 0행**인 kind 에서만(부분 매칭 개선은 범위 밖) Bedrock 에 쿼리 1개를 요청하고, (a) static read-only 검사 (b) **그 인스턴스 스키마 어휘 언급** (c) 비어 있지 않은 dry-run 응답을 모두 통과해야 캐시. 실패가 일시적이면 `{version}:retryNw<주차>` 로 저장해 다음 실행이 재시도(주당 3회 상한, 소진 시 그 주만 park). 생성 행은 Explore 칩 전용 — 진단 리포트 경로에서 제외 | ADR-007 |
