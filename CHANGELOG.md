...
- Dashboard freshness & at-a-glance decomposition: the header gains an admin-only 'Sync all' action that dispatches one all-types inventory sync on demand (async batch semantics disclosed in place — queued/admin-only/sync-disabled states; data lands via the normal Refresh minutes later, no optimistic mutation), and the resource tiles gain v1's micro-stat sublines (EC2 running/stopped, Lambda runtimes/>300s, ECS services/tasks, ECR scan-on-push/immutable, EKS ready-nodes/pods/deploys from the live fleet, CloudFront enabled, VPC subnets/NAT/TGW, WAF rule groups/IP sets, EBS GiB/unencrypted, S3 public/versioning-off, RDS Multi-AZ/unencrypted, IAM no-MFA, SG open ingress) — shared with the group-overview pages from one map so the two surfaces cannot drift; a subline renders only once its inputs are loaded and drops when a split is absent, and types without decomposition data keep plain counts (no extra SQL — existing summary splits/byType/fleet data only).
...
- 대시보드 최신화·한눈 분해: 헤더에 관리자 전용 '전체 동기화' 버튼 추가 — 전체 타입 인벤토리 sync를 온디맨드로 1회 dispatch(비동기 배치 시맨틱을 그 자리에서 공지 — 큐잉/관리자 전용/sync 비활성 상태 표시, 데이터는 수 분 후 일반 Refresh로 반영, 낙관적 갱신 없음). 리소스 타일에는 v1의 마이크로스탯 서브라인 복원(EC2 running/stopped, Lambda runtimes/>300s, ECS services/tasks, ECR scan-on-push/immutable, EKS ready 노드/파드/디플로이[라이브 fleet], CloudFront enabled, VPC subnets/NAT/TGW, WAF rule groups/IP sets, EBS GiB/미암호화, S3 public/versioning-off, RDS Multi-AZ/미암호화, IAM no-MFA, SG open ingress) — 그룹 overview 페이지와 하나의 맵을 공유해 두 표면이 드리프트하지 않으며, 서브라인은 데이터 로드 후에만 표시되고 split 부재 시 해당 서브라인만 생략, 분해 데이터가 없는 타입은 카운트만 표시(추가 SQL 없음 — 기존 summary splits/byType/fleet 데이터만 사용).
