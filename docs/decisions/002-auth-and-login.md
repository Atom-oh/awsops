...
> **Amendment (2026-08-19, PWA install):** `is_public()`'s allowlist grew by 5 exact-match static-asset paths — `/manifest.webmanifest`, `/apple-touch-icon.png`, `/icon-192.png`, `/icon-512.png`, `/icon-512-maskable.png`. iOS/iPadOS fetches the web app manifest and its icons to build the home-screen install prompt *without* sending the `awsops_token` cookie, so these paths must stay reachable unauthenticated the same way `/icon.svg` already was. `is_public()` matches on the CloudFront request's path only, so a query string is ignored (e.g. `/manifest.webmanifest?x=1` is also public) — harmless here since every entry is a static, parameter-free asset with no prefix widening (exact `uri in (...)` match), carries no session or account data, and there is no catch-all route for them to fall through to if the match fails — the same posture as the pre-existing favicon carve-out, just five more static files. `web/app/manifest.test.ts` pins each path against both the edge template and the actual `public/` files (3-way lockstep). `/manifest.webmanifest` is served by a route handler (`web/app/manifest.ts`); it is reachable unauthenticated but is *not* CDN-cached — `edge.tf` applies `Managed-CachingDisabled` to every path outside `/_next/static/*`, this one included, so each request reaches origin. Because it never calls `verifyUser()` either, it must stay a static-constant route: no per-user, per-session, or environment-derived value may ever enter its response.
>
> **개정 (2026-08-19, PWA 설치):** `is_public()` 허용목록에 정적 자산 경로 5개(정확히 일치)가 추가됐다 — `/manifest.webmanifest`, `/apple-touch-icon.png`, `/icon-192.png`, `/icon-512.png`, `/icon-512-maskable.png`. iOS/iPadOS는 홈 화면 설치 프롬프트를 만들 때 `awsops_token` 쿠키 없이 web app manifest와 아이콘을 가져오므로, 기존 `/icon.svg`와 같은 이유로 미인증 상태에서도 도달 가능해야 한다. `is_public()`은 CloudFront 요청의 경로만 매칭하므로 쿼리스트링은 무시된다(예: `/manifest.webmanifest?x=1`도 공개로 판정됨) — 각 항목이 쿼리 파라미터를 쓰지 않는 정적 자산이라 무해하다. prefix 확장은 없고(정확 일치 `uri in (...)`) 세션·계정 데이터를 담지 않으며, 매칭 실패 시 떨어질 catch-all 라우트도 없다 — 기존 파비콘 예외와 동일한 성격의 정적 파일 5개가 늘었을 뿐이다. `web/app/manifest.test.ts`가 엣지 템플릿과 실제 `public/` 파일을 3자 lockstep으로 고정한다. `/manifest.webmanifest`는 라우트 핸들러(`web/app/manifest.ts`)가 서빙하며 미인증 상태로 도달 가능하지만 CDN에 캐시되지는 않는다 — `edge.tf`는 `/_next/static/*`를 제외한 모든 경로에 `Managed-CachingDisabled`를 적용하므로 이 경로도 매 요청이 origin까지 도달한다. `verifyUser()` 역시 호출하지 않으므로, 사용자별·세션별·환경별로 달라지는 값이 응답에 절대 들어가지 않는 정적-상수 라우트로 유지해야 한다.

### 2. 로그인 — 자체 호스팅 `/login` 폼 (1차 경로) / Login — self-hosted `/login` form (primary path)

로그인 주 경로는 **자체 호스팅 `/login` 폼**이다(AgentCore teal 테마, 활성 테마 추종; `ShellGate`가 bare 경로(`/login`, 그리고 인쇄용 뷰 `/ai-diagnosis/report` [2026-09-01])를 제외한 모든 경로에 앱 셸을 탑재). 폼은 BFF `POST /api/auth/login`을 호출하고, BFF는 Cognito **`InitiateAuth(USER_PASSWORD_AUTH)`**를 **무서명 공개 오퍼레이션**으로 plain fetch(`cognito-idp.{region}.amazonaws.com`, `X-Amz-Target: AWSCognitoIdentityProviderService.InitiateAuth`, `Content-Type: application/x-amz-json-1.1`) 호출한다 — SDK·IAM·task-role 자격증명 불필요. 성공 시 Cognito가 반환한 `IdToken`을 `awsops_token` 쿠키로 발급한다("keep me signed in" ✓ → `Max-Age=43200`[12h, id_token 수명], ✗ → 세션 쿠키). 오류는 단일 코드로 수렴해 계정 존재를 노출하지 않는다: `NotAuthorizedException`/`UserNotFoundException` → `invalid_credentials`, 모든 `ChallengeName` → `challenge`(403, "관리자에게 문의"), 네트워크/5xx → `unavailable`(502).

The primary login path is the **self-hosted `/login` form** (AgentCore teal theme, follows the active theme; `ShellGate` mounts the app shell on every route except the bare set (`/login`, and the print view `/ai-diagnosis/report` [2026-09-01])). The form calls BFF `POST /api/auth/login`, which invokes Cognito **`InitiateAuth(USER_PASSWORD_AUTH)`** as an **unsigned public operation** via plain fetch (`cognito-idp.{region}.amazonaws.com`, `X-Amz-Target: AWSCognitoIdentityProviderService.InitiateAuth`, `Content-Type: application/x-amz-json-1.1`) — no SDK, no IAM, no task-role credentials. On success it mints the Cognito-issued `IdToken` as the `awsops_token` cookie ("keep me signed in" ✓ → `Max-Age=43200` [12h, the id_token lifetime], ✗ → session cookie). Errors collapse to single codes so account existence is never disclosed: `NotAuthorizedException`/`UserNotFoundException` → `invalid_credentials`, any `ChallengeName` → `challenge` (403, "contact your administrator"), network/5xx → `unavailable` (502).
