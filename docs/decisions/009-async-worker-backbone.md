...
- **종료 절차(순서 고정)** / **Termination procedure (ordered)**:
  ...
  2. `make backfill-owner-sub` 가 **계획만** 쓴다(아무것도 바꾸지 않음). 운영자가 확신할 수 없는 항목을
     ...
     half-transferred ownership table is worse than an untouched one. **Quiesce `awsops-v2-schedule-dispatcher` (EventBridge `disable-rule`) for the duration of the
     apply**, and re-enable it only after the residual check reads 0: the dispatcher reads an owner
