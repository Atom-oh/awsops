...
**권한 분리:** Agent Space·Integration/MCP 등록(egress/자격증명/SSRF 표면) = admin. **폼 기반** Skill·Agent 작성·조합 = 일반 사용자(read-only 범위; disabled-by-default + admin-only enable). **zip 업로드는 cosign 검증 전까지 admin 전용**(ADR-031 Addendum #6 공급망 표면 유지). ADR-031 admin-only authoring 대비 의도적 완화 — §16에 기록.
...
**Permission split (confirmed — a deliberate revision of ADR-031, recorded in §16):** Agent Space + Integration/MCP registration (egress endpoints + credentials, the SSRF/egress surface) = **admin** (ADR-023 gate); **form-based** Skill + Agent authoring/composition within read-only tool scope = **any authenticated user** (ADR-031 was admin-only authoring; this relaxation is safe because authored objects are disabled-by-default, server-side-allowlisted, SAFEGUARD_LINE-bounded, and **admin-only to ENABLE**). **Externally-sourced zip uploads stay admin-only** (the supply-chain surface ADR-031 Addendum #6 scoped to admins) until cosign verification (§11). Enabling a `READ_WRITE` integration or wiring a write action = **admin + the mutating-gate 4-eyes** (or the logged ADR-029 single-operator escape hatch).
...
- **`external-obs` / gateway count (THREE sources diverge — reconcile before ratification):** (corrected) `external-obs` IS already seeded as a 9th key in `scripts/v2/agentcore/catalog.py` (GATEWAYS list + DESCRIPTIONS, "registry built in P3", no Lambda target yet); but `agent.py` `SKILL_BASE` instead carries a `diagnostics` persona (no `external-obs`); and **ADR-004's canonical count is 8 role-based gateways**. The `agents` catalog seed has 8. Decide: external observability = **Integrations axis** (the §7 lean: external obs = ADR-011/Integrations, internal CloudWatch = `monitoring` gateway), and reconcile `catalog.py` external-obs vs `agent.py` diagnostics vs ADR-004=8 (update ADR-004 if the count changes).
- **ADR-008 account identity in v2:** the whole per-account Agent Space design keys on an account id that v2 currently hard-wires to `currentAccountId()='self'` (§3); ADR-008's account-enumeration (`AccountConfig`/Steampipe aggregator) is a v1 mechanism not yet rebuilt in v2. **Per-account scoping degrades to single-account ('self') until a v2 successor to ADR-008 lands** — state this assumption rather than implying multi-account works today.
- **ADR-035 (K8sGPT in-cluster diagnosis):** a read-only in-cluster K8s MCP source under the container-section agent (P3) — decide whether it is a section-gateway tool or an Integrations-axis egress READ connector; do not duplicate its registration.
...
|---|---|---|
| **ADR-031 Addendum #6** (admin-only authoring/uploads) | **deviation** | ADR-039 lets non-admins do **form-based** skill/agent authoring within read-only scope (safe: disabled-by-default + server allowlist + SAFEGUARD + admin-only ENABLE). **Zip uploads stay admin-only until cosign** (Addendum #6 retained for the supply-chain surface). §10/§11. |
