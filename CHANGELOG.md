...
- Grouped cost charts: the ECS Tasks page gains v1's Cost by Service chart (CPU vs Memory daily-cost split per service from the shared estimator constants — FARGATE tasks only, top 10; EC2/serviceless tasks are excluded rather than charted without an estimate), and the EKS container-cost page gains the Node Daily Cost + Pod Count chart (from the page's own OpenCost node allocation + pods list, no new fetch) — both on a new multi-series grouped-bar primitive whose per-series scaling replaces v1's dual axis (value labels carry the real numbers/units).
...
- 그룹 비용 차트: ECS Tasks 페이지에 v1의 Cost by Service 차트(서비스별 CPU vs Memory 일일 비용 분해 — 공용 추정 상수 사용, FARGATE 한정 상위 10, EC2/서비스 없는 태스크는 추정 없이 그리지 않고 제외), EKS 컨테이너 비용 페이지에 Node별 일일 비용 + Pod 수 차트(페이지 자체의 OpenCost 노드 할당 + pods 목록 사용, 신규 fetch 없음) 추가 — 두 차트 모두 신규 멀티 시리즈 그룹 바 프리미티브 기반으로, v1의 이중 축을 시리즈별 자체 스케일로 대체(값 라벨이 실제 수치/단위 표기).
