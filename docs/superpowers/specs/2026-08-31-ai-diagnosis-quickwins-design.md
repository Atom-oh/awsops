...
리포트 ID), 생성 중 섹션 체크리스트 그리드(완료 체크/진행 스피너/대기), 빈 상태의 섹션
프리뷰 그리드, 히스토리 행 인라인 MD/DOCX 다운로드 링크. 서버 측 변경은 워커
`update_progress`의 progress JSONB에 `completed`(완료 섹션 제목 배열) 추가 1건뿐 —
스키마/API/Terraform 변경 없음.
...
- Stats bar: on a completed opened report, one line under the download row:
  `섹션 N개 · 소요 mm:ss · 리포트 #id`. Elapsed = `finished_at − created_at` — BUT the list
  row does not carry `finished_at` today. The report GET (`/api/diagnosis/[id]`) returns the
  row; verify and surface `finished_at` in the list/report response if already selected;
  otherwise compute only when available and omit the 소요 segment (honest-degrade, no fake
  duration). Section count = `summary.sections` (already in `ReportSummary`).
...
- UI (`ProgressPanel`): when `progress.completed` exists, render a grid of ALL expected
  section titles for the run's tier — completed → green check, the current `section` →
  spinner, rest → dimmed pending. The expected list comes from `progress.total` + a static
  catalog (below). When `completed` is absent (old in-flight rows), keep today's bar-only UI.
- Static catalog: `web/lib/diagnosis-sections.ts` — a hand-mirrored list of the worker's
  section keys/titles per tier (8 base + 7 deep + intended_vs_actual), with a lockstep
  comment pointing at `scripts/v2/workers/diagnosis/sections.py` (same convention as the
  other documented manual lockstep sites). Titles listed in ko with the en/zh/ja deep-title
  variants resolved via the worker's TITLES_I18N equivalents — the grid matches by list
  order/count (not string equality) against `progress.completed` to avoid language-fragile
  matching: completed count marks the first N grid entries in catalog order ONLY when that
  is provably safe. It is NOT (render is concurrent, completion order ≠ catalog order), so
  match by title string against BOTH the ko catalog title and its localized variants; an
  unmatched completed title (future drift) simply doesn't check a box — never a wrong check.
...
- The empty state (`리포트를 선택하거나 "진단 실행"을 누르세요`) gains a scope paragraph +
  a chip grid of the section titles from the same static catalog (8 base chips + 7 deep chips
  marked `Deep`), so users see what a diagnosis covers before running one. Icons: one lucide
  icon per section group (reuse existing icon imports; no new deps).
...
- `ProgressPanel`: timer renders mm:ss from created_at; checklist grid states (completed
  check / current spinner / pending dim); absent `completed` → legacy bar only.
