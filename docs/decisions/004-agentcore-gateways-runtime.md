...
>
> external-obs(외부 관측성)는 **Integrations 축**(§3)의 일부로, 외부 데이터소스 커넥터(Prometheus·ClickHouse 람다 target; Loki/Tempo/Mimir는 monitoring 잔류, Datadog/Dynatrace/New Relic은 ADR-017 벤더 호스팅 프리셋 활성화 시 여기에 등록 — 구서술 정정 2026-08-06)를 호스팅하는 라우팅 섹션이다. 챗 섹션 키는 `observability`이며 `agent.py`에서 `external-obs` 게이트웨이로 별칭 매핑된다.
>
...
>
> **Follow-up (2026-08-05, ADR-017 재개정 — anti-drift 정합화):** 위 amendment가 "재-ack 없이 흡수 — 알고서 수용한 잔여 리스크"라고 기록한 사유는 ADR-017의 2026-08-05 재개정으로 **해소**됐다 — 남은 벤더 호스팅 3종(Datadog·Dynatrace·New Relic, 자체 호스팅 5종은 카탈로그에서 제거됨)에 런타임 fail-closed 툴 allowlist(`OFFICIAL_MCP_TOOL_ALLOWLIST_JSON` → `agent.py`가 교집합, 공란/env부재 ⇒ 툴 0개)가 코드로 강제되어 벤더의 write 툴 추가가 더 이상 무심사로 흡수되지 않는다. **위 amendment 중 그대로 유효한 것과 stale해진 것을 구획한다**: (유효) SSRF/DNS-rebinding이 managed-egress 경로에서 connect-time에 코드로 강제 불가능하다는 것 — allowlist는 어떤 툴이 노출되는지를 제한할 뿐, 그 툴 호출이 어디로 나가는지의 검증을 대신하지 않는다. (stale) "RFC1918은 self-host 프리셋 때문에 무조건 허용" — 자체 호스팅 프리셋은 카탈로그에서 전부 제거되어 현재 0건이고, RFC1918 허용과 사설-IP-리터럴 강제(`_host_pin_violation`)는 미래의 operator-asserted 프리셋을 위한 보존 규칙이다(ADR-017 §Trade-offs). 영어판 amendment의 "host is operator-asserted rather than code-pinned" 서술도 stale — 호스트 핀은 2026-07-31판부터 코드 강제이며(한국어판은 정정됨) 이번 개정에서도 유지된다. `official_mcp_enabled`는 일반 GATED(BASELINE §2). ClickHouse의 stdio 내장(`CLICKHOUSE_OFFICIAL_MCP`)은 이 리스크와 무관한 별도 메커니즘이며 별도 사유로 여전히 do-not-enable/FROZEN이다(자체 람다의 SSRF/read-only 1차 방어를 대체 없이 상실 — ADR-017 §Trade-offs, BASELINE §2).
>
> **Follow-up (2026-08-05, ADR-017 re-amendment — anti-drift reconciliation; English mirror of the paragraph above, added 2026-08-06 — review MAJOR: the follow-up was Korean-only while this amendment's English text still asserted the reversed status):** the "knowingly accepted residual risk" recorded above (a vendor write tool absorbed with no re-ack) is **resolved** by ADR-017's 2026-08-05 re-amendment — the 3 remaining vendor-hosted presets (Datadog/Dynatrace/New Relic; the 5 self-hosted presets are removed from the catalog) sit behind a runtime fail-closed tool allowlist (`OFFICIAL_MCP_TOOL_ALLOWLIST_JSON` → intersected in `agent.py`, empty/unset ⇒ zero tools), so a vendor-added write tool is no longer absorbed unreviewed. Scoping what stands vs is stale in the English amendment text above: (stands) SSRF/DNS-rebinding cannot be code-enforced at connect time on the managed-egress path — the allowlist limits WHICH tools exist, not WHERE a call goes. (stale) "RFC1918 is unconditionally allowed because several presets are deliberately self-hosted" — zero self-hosted presets remain; the RFC1918 allowance and the private-IP-literal pin (`_host_pin_violation`) are preserved rules for any future operator-asserted preset. (stale) "the endpoint host is operator-asserted rather than code-pinned" — the host pin has been code-enforced since the 2026-07-31 revision and survives this one. `official_mcp_enabled` is ordinary GATED (BASELINE §2); ClickHouse's stdio embedding (`CLICKHOUSE_OFFICIAL_MCP`) is a separate mechanism, still do-not-enable/FROZEN for a different reason (it drops the in-house lambda's SSRF/read-only first-line defenses with no replacement — ADR-017 §Trade-offs, BASELINE §2).
