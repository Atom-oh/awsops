...
5. **라이브 AWS 조회 = AgentCore MCP Lambda 도구.** v2에 라이브 Steampipe는 없다. flag-gated warm Steampipe 인벤토리 sync(Fargate + sync Lambda → Aurora)는 배치 로더로만 존재하며 기본 off다. v1의 pg-Pool/VPC-Lambda 호스트 가정(ADR-001/005)은 v2에서 무의미.
   ...
   **2026-08-31 롤아웃 노트(ADR-021):** Phase 1 쿼터 가드, durable last-success, partial/degraded freshness 공개는 저장소에 구현됐다. 성공한 0-row는 이후 실패에도 보존되고 expected account 도달 불가 시 last-good row를 유지한다. 이 변경을 수행한 에이전트는 apply를 실행하지 않았으며 실제 배포는 controller에서 별도 확인한다. 현재 ops gateway의 제한된 Aurora `inventory-read-target`이 direct domain inventory/configuration target과 공존한다. Phase 2가 domain-aware Aurora coverage를 확장하고 direct target을 retirement하므로 Aurora-only는 아직 live가 아니며 ADR-005도 바뀌지 않는다.
   (**2026-08-31 rollout note (ADR-021):** Phase 1's quota guard, durable last-success, and partial/degraded freshness disclosure are implemented in the repository. Successful zero-row history survives later failures, and unreachable expected accounts preserve last-good rows. The agent making this change did not run apply; actual deployment must be verified separately with the controller. The ops gateway's limited Aurora `inventory-read-target` currently coexists with direct domain inventory/configuration targets. Phase 2 expands domain-aware Aurora coverage and retires direct targets, so Aurora-only is not live; ADR-005 is unchanged.)
