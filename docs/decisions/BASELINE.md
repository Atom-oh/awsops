...
| GATED operator private migrations | `github_actions_migration = null` | Default absent; dedicated ARM64 Fargate migration task/roles in existing private networking. Only the task can read approved database secrets. CI registers/runs the owned migration family and verifies source, actual digest, exit and nonce-bound receipt. No new ingress, state read or product mutation path. | ADR-005 operator boundary; [runbook](../runbooks/private-ci-migrations.md) |
