...
v1(EC2, `data/*.json`)의 **고가치 이력 4종**을 v2 Aurora의 ADR-030 앱상태 테이블로 일회성·**멱등** 적재한다. Steampipe에는 옮길 영속 데이터가 없으며(라이브 FDW 엔진), v2 라이브 인벤토리 sync(`inventory_resources`)와는 무관하다.
One-time, **idempotent** load of v1's four high-value history stores into v2's ADR-030 Aurora app-state tables. Steampipe holds no durable data (live FDW engine); this is unrelated to the v2 live inventory sync (`inventory_resources`).
...
- ADR: **030**(ECS Fargate + Aurora 앱상태 7테이블), **037**(v2 파운데이션). 스펙 = `docs/superpowers/specs/2026-06-12-v1-to-v2-aurora-backfill-design.md`.
