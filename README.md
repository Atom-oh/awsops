...
[![Upstream version](https://img.shields.io/badge/upstream-v0.9.0-green.svg)](https://github.com/whchoi98/awsops/releases/tag/v0.9.0)
...
Version provenance: `v0.9.0` identifies the [upstream v2 release published on 2026-08-22](https://github.com/whchoi98/awsops/releases/tag/v0.9.0); this repository records its local integration changes under `[Unreleased]` in [CHANGELOG.md](CHANGELOG.md).

버전 출처: `v0.9.0`은 [2026-08-22에 게시된 upstream v2 릴리스](https://github.com/whchoi98/awsops/releases/tag/v0.9.0)를 가리키며, 이 저장소의 로컬 통합 변경은 [CHANGELOG.md](CHANGELOG.md)의 `[Unreleased]`에서 추적합니다.
...
Stats: 41 pages, 100 API routes, 110 components (`web/`), 21 consolidated ADRs, Terraform-managed (`terraform/v2/foundation`, no CDK).
...
```
awsops/
  web/                    # Next.js 14 thin-BFF: 41 pages, 100 API routes, 110 components
...
PR-review fixtures cover all 12 nonce-bound JSON report frames, decoded reports without
tool chatter, literal tool/marker examples, strict CLI exits, retries, hard kills, and
Pod Identity preflight without live AWS/AI calls. The CI preflight validates
...
The 100 API routes live under `web/app/api/`. Key routes: `health` (public), `stream` (SSE chat), `db` (Aurora ping), `jobs` (+`/[id]`, async job submission/status), `security`, `compliance`, `auth/login`. See the docs site for user-facing guidance and [docs/decisions/BASELINE.md](docs/decisions/BASELINE.md) for architectural decisions.
...
현황: 41 페이지, 100 API 라우트, 110 컴포넌트(`web/`), 21개 통합 ADR, Terraform 관리(`terraform/v2/foundation`, CDK 없음).
...
```
awsops/
  web/                      # Next.js 14 thin-BFF: 41 페이지, 100 API 라우트, 110 컴포넌트
...
PR 리뷰 fixture는 실제 AWS/AI 호출 없이 12개 셀의 nonce 결합 JSON 보고서 프레임,
도구 출력 없이 디코딩된 보고서, 도구·마커 예문, CLI 종료 상태, 재시도, 하드킬과
Pod Identity preflight를 검증한다. CI preflight는 panel/chair 전에
기존 provider의 유효성만 확인하며, 자격 증명을 export하거나 lease 갱신을 강제하지 않아
모델 CLI의 기존 SDK 자동 갱신 경로를 유지한다.

## API 문서

100개 API 라우트가 `web/app/api/`에 있습니다. 주요 라우트: `health`(공개), `stream`(SSE 채팅), `db`(Aurora ping), `jobs`(+`/[id]`, 비동기 작업 제출/상태), `security`, `compliance`, `auth/login`. 사용자 가이드는 docs site를, 아키텍처 결정은 [docs/decisions/BASELINE.md](docs/decisions/BASELINE.md)를 참고하세요.
