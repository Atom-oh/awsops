...
- **게이트웨이에 연결된 섹션 예시**: Network, Container, Data, Security, Cost, Monitoring, IaC, Ops, Observability. 전체 \`/\` 명령 목록은 이 예시보다 넓으며, 실제 도구 가용성은 배포와 연동 설정에 따릅니다.
...
**연동(Integrations) → Agents & Skills**에서 **Custom Agents & Skills 열기 →**를 선택하거나 \`/customization\`으로 이동합니다. 관리자 권한이 필요합니다.
...
연동 허브의 **Agents & Skills**에서 \`/customization\`을 열어 진행합니다:
1. **/integrations → Datasources**에서 Prometheus를 설정하고 **연결 테스트**를 실행한 뒤 저장합니다. **탐색 →**에서 \`up\` 같은 작은 읽기 전용 쿼리로 실제 접근을 확인하세요. 설정 저장 여부는 연결 성공을 뜻하지 않습니다. 자격증명은 서버에서 관리합니다.
...
> 참고: AWSops는 **AWS 리소스를 변경하지 않습니다**. 에이전트는 외부 데이터를 **읽어** 분석/진단하며, 외부 기록 write(티켓·메시지)는 별도 거버넌스(DLP·사람의 승인·flag-OFF) 하에서만 동작합니다.
