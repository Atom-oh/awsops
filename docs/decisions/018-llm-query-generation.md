...
- **캐시 크기 폴백** — 커넥터 캡은 이름 **개수**(Prometheus/Mimir 3000)이므로 긴 이름 환경은 캐시 행
  바이트 상한(256KB)을 넘을 수 있다 → 라우트가 메트릭 목록을 반씩 줄여 맞춘 뒤 `truncated: true`로
  저장(행 없음 → 요청마다 재수집을 피함). 구 캡(정확히 500개·`truncated`·PromQL 종류) 스냅샷은 백그라운드
  재수집 대상이며, 재수집은 인스턴스당 쿨다운(10분)으로 보호된다.
