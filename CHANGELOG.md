...
- Group overview tiles: per-type micro-stat sublines (v1 parity) — EC2 running/stopped, Lambda runtimes and >300s timeouts, EBS total GB and unencrypted, RDS Multi-AZ/unencrypted, ECR scan-on-push/immutable, S3 public/versioning-off, IAM no-MFA, SG open-ingress, alarms in ALARM, CloudFront enabled, and VPC subnet·NAT composition — computed in the existing single summary aggregation (no new AWS calls) and hidden, not zeroed, while loading.
...
- 그룹 개요 타일: 타입별 마이크로스탯 서브라인(v1 패리티) — EC2 running/stopped, Lambda 런타임 수·>300s 타임아웃, EBS 총 GB·미암호화, RDS Multi-AZ/미암호화, ECR scan-on-push/immutable, S3 public/versioning off, IAM no-MFA, SG open-ingress, ALARM 상태 경보, CloudFront enabled, VPC 서브넷·NAT 구성 — 기존 단일 summary 집계 쿼리에서 계산(신규 AWS 호출 없음), 로딩 중에는 0을 지어내지 않고 숨김.
