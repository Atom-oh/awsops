...
**게이트 수학 (검증 라운드에서 65케이스 전수 수동 트레이스):** regex-only 베이스라인 = **44/65 ≈ 67.7%** (무매칭/교차도메인 21케이스 중 `ops`가 정답인 2개만 우연 정답). `+15pp` 게이트 = 82.7% < SLO 85% → **바인딩 게이트는 85%** — Haiku가 65케이스 중 ≥56개를 맞추면 통과(도전적·달성 가능). 초안(52케이스)의 베이스라인 82.7%는 +15pp 게이트를 수학적으로 달성 불가(hybrid ≥97.7%)로 만들었음 — 무매칭 13케이스 추가로 복구.
...
Expected: PASS + 콘솔에 `regex-only baseline: 67.7% (44/65)` (검증 라운드 수동 트레이스와 일치해야 함 — 크게 다르면 케이스 오타 의심). **만약 sanity bounds(0.3~0.85)를 벗어나면** 골든셋의 모호/교차도메인 케이스 비율을 조정(실패모드 케이스 추가)하고 재실행 — 케이스 수정은 허용, 게이트 수치(0.85/+15pp)는 수정 금지.
