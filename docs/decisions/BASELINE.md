...
| **GATED** | SG Rules & Usage — 일일 rule 인벤토리 + Athena 기반 90일 트래픽 증거 (`sg_rule_inventory`/`sg_rule_activity`/`sg_rule_source_validate` job) | `sg_rule_activity_enabled` | OFF | `workers_enabled` 선행. Athena/Glue는 read/query 동사만, mutating 동사(CreateTable/CreateWorkGroup 등) 없음. **정정(2026-08-19)**: S3는 read-only가 아니다 — ADR-019 §Decision이 `s3:PutObject`/`AbortMultipartUpload`를 고객 사전설정 워크그룹 결과 prefix에만 명시적으로 승인한다(Athena 쿼리 실행 자체가 요구하는 필수 동작, §ADR-019 §3b). 그 prefix 밖 write와 모든 `s3:DeleteObject`는 금지. `sg_rule_activity_max_query_bytes`(기본 100 GiB)로 스캔 예산 제한 | ADR-019 |
...
| [019](019-athena-flow-log-query-classification.md) | Athena Flow Log 조회 분류 (read-only 불변식 내부) | Athena/Glue read-query(StartQueryExecution/GetQueryResults/StopQueryExecution 등)는 기존 CloudWatch Logs Insights 패턴과 동형 — ADR-005 완화·ADR-007 티어 불필요. 결과 write는 고객 사전설정 워크그룹 위치로 한정, AWSops가 버킷 생성/관리 안 함. 워커 role의 `AWSopsReadOnlyRole` cross-account assume은 기존 web/Steampipe/agent 패턴의 새 호출자일 뿐. `network_path_check_enabled`/`sg_rule_activity_enabled`는 이 결정에 따라 일반 GATED로 §2 등록 | 보안·비용 |
