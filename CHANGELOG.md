...
- Datasource Explore/management parity batch: curated example-query and natural-language prompt chips for all 8 connector kinds, a dedicated Loki log-stream viewer (timestamps, label badges, scrollable pane), 7d/30d time-range presets (the query API's window bound widens to 30d while the 5000-point density cap remains the cost guard), a result metadata bar (rows/series · execution ms · query language · shape), a dismissible "AI generated from …" banner after NL→query drafting, KPI tiles and a manual refresh button on the management tab, a per-row "AI로 진단" deep link that prefills the assistant composer (`/assistant?q=`, review-only — never auto-sends), and proportional duration bars on Tempo/Jaeger trace results.
...
- 데이터소스 Explore/관리 패리티 배치: 8개 커넥터 타입 전부에 큐레이트 예제 쿼리·자연어 프롬프트 칩, Loki 전용 로그 스트림 뷰어(타임스탬프·라벨 배지·스크롤 패널), 7d/30d 기간 프리셋(쿼리 API window 상한을 30d로 확장하되 5000-포인트 밀도 캡이 비용 가드로 유지), 결과 메타데이터 바(행/시리즈 수 · 실행 ms · 쿼리 언어 · 형태), NL→쿼리 생성 후 닫을 수 있는 "AI 생성됨" 배너, 관리 탭 KPI 타일과 수동 새로고침 버튼, 행별 "AI로 진단" 딥링크(`/assistant?q=` 컴포저 프리필 — 검토 전용, 자동 전송 없음), Tempo/Jaeger 트레이스 결과의 비례 duration 바 추가.
