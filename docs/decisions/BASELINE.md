...
> **인벤토리 쿼터 격리 (ADR-021, 2026-08-31):** Phase 1의 전역 Steampipe limiter와 sync backpressure는 **코드 구현됨, 프로덕션 apply 미수행**이다. **현재 AgentCore inventory/configuration 동작은 Phase 2 catalog retirement가 배포될 때까지 직접 AWS API 호출로 유지된다.** 목표는 quota-limited Steampipe→Aurora→domain MCP이며, Phase 2/3은 아직 live가 아니다. 이 결정은 ADR-005 FROZEN을 완화하지 않는다.
>
> **Inventory quota isolation (ADR-021, 2026-08-31):** Phase 1's global Steampipe limiter and sync backpressure are **implemented in code, with no production apply performed**. **Current AgentCore inventory/configuration behavior remains direct AWS API calls until Phase 2 catalog retirement is deployed.** The target is quota-limited Steampipe→Aurora→domain MCP; Phases 2/3 are not live. This does not relax ADR-005 FROZEN.
...
| [021](021-quota-isolated-inventory-reads.md) | 쿼터 격리 인벤토리 읽기 | **Phase 1 코드 구현·프로덕션 apply 미수행**: Steampipe `awsops_global` limiter(4/4/2) + sync reserved concurrency 4 + 15분 async expiry. **현재 direct AgentCore inventory/configuration API 호출은 Phase 2 catalog retirement 전까지 유지**; 목표 Aurora MCP와 Phase 3 cache retirement는 pending, ADR-005 FROZEN 불완화 | 신뢰성·운영우수성·성능·보안 |
