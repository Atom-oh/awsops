...
- 상세 패널 렌더링 퀵윈: EBS attachment에 DeleteOnTermination 플래그(설정 시 — 인스턴스와 함께 볼륨 삭제), EBS 상세에 암호화 판정 배너(green+KMS 키 / red+암호화 사본 권고; 미상은 표시 안 함)와 미연결 볼륨 유휴 비용 힌트, ECS 클러스터 settings를 raw JSON 대신 라벨–값 행으로 렌더링.
