...
벤치마크 실행이 완료되면 벤치마크명·범위(scope)·전체/통과/실패(Alarm) 건수·통과율과 `/compliance` 링크가 담긴 SNS 이메일이 발송됩니다. AI 진단 알림과 동일한 SNS 토픽/구독을 사용하며(`diagnosis_notify_enabled` 게이트), 관리자용 일시중지 스위치(진단 이메일 일시중지)를 켜면 함께 중지됩니다. 알림 실패는 벤치마크 결과에 영향을 주지 않습니다(best-effort).
