...
| **P3** | 9+1 에이전트 + 우측 도킹 챗 UI + OpenCost 설치 버튼(ADR-029 mutating) + EKS 조회 | 🔜 backlog |
...
- 신규 대형 기능은 **count/flag 게이트**로: `agentcore_enabled`, `workers_enabled`(둘 다 기본 false → `plan`=No changes, $0). 토글 전 기본은 비활성.
...
## ADR
`docs/decisions/ADR-*.md` (001–036). v2 관련: **030**(ECS Fargate + Aurora) 채택; **033**(AIOps LLM 비용 최적화), **034**(알림 자동 RCA 라이트백), **035**(K8sGPT 하이브리드 인클러스터 진단), **036**(변경·조치 실행 substrate — SSM Automation+Change Manager × P2 하이브리드)은 **멀티AI 합의 리뷰(ACCEPT-WITH-CHANGES)로 Accepted (2026-06-09)**; **029**(변경 작업 프레임워크 — 메커니즘은 036이 정제, 6대 통제 유지), **031**(런타임 커스터마이즈 에이전트), **032**(자율 인시던트 라이프사이클)은 Proposed. 신규 ADR 번호 = 최고번호+1. ADR 인덱스/정정 노트는 `docs/decisions/CLAUDE.md`.
...
| **P3** | 9+1 agents + right-dock chat UI + OpenCost install button (ADR-029 mutating) + EKS query | 🔜 backlog |
...
- Gate large new features with count/flags: `agentcore_enabled`, `workers_enabled` (both default false → `plan` = No changes, $0).
...
## ADR
`docs/decisions/ADR-*.md` (001–036). v2-relevant: **030** (ECS Fargate + Aurora) adopted; **033** (AIOps LLM cost optimization), **034** (alert auto-RCA write-back), **035** (K8sGPT hybrid in-cluster diagnosis), **036** (remediation execution substrate — SSM Automation+Change Manager × P2 hybrid) are **Accepted (2026-06-09) via multi-AI consensus review (ACCEPT-WITH-CHANGES)**; **029** (mutating-action framework — mechanism refined by 036, six controls retained), **031** (runtime-customizable agents), **032** (autonomous incident lifecycle) are Proposed. New ADR number = highest + 1. ADR index/correction notes: `docs/decisions/CLAUDE.md`.
