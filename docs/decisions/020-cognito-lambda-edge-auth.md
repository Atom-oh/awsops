...
- **2026-06-03 — edge JWT verification hardened to full RS256 (v2, commit `8313b0e`).** The v1 Decision/Rationale above frames the Lambda@Edge handler as a "short OAuth2 + JWT-**exp** check" using stdlib `base64`/`json`, and `auth-utils.ts` decodes "without signature verification **because Lambda@Edge has already enforced signature validity**." A base64 decode + expiry check does **not** verify the token signature, so in v1 that whole-chain claim was not actually backed by signature verification — a decode-only app trusting an edge that itself only checked `exp` would accept a forged/altered JWT. The v2 branch (`feat/v2-architecture-design`) hardens the edge handler to **JWKS-based RS256 signature verification** (plus issuer/audience checks, OAuth `state`, and PKCE for the public client — dropping the client secret). Only with v2 is the "decode-only, trusts edge" model genuinely sound. Operators on v1 should treat edge auth as **exp-only** until v2 is deployed. / **2026-06-03 — 엣지 JWT 검증을 완전한 RS256으로 강화(v2, 커밋 `8313b0e`).** 위 v1 결정/근거는 핸들러를 stdlib `base64`/`json` 기반 "OAuth2 + JWT **exp** 검사"로 기술하고 `auth-utils.ts`는 "엣지가 서명 유효성을 이미 강제했으므로 서명 검증 없이" 디코드한다고 한다. 그러나 base64 디코드 + 만료 검사는 **서명을 검증하지 않는다** — v1에서는 이 전제가 실제 서명 검증으로 뒷받침되지 않아, exp만 검사하는 엣지를 신뢰하는 decode-only 앱은 위조/변조된 JWT를 수용할 수 있었다. v2 브랜치는 엣지 핸들러를 **JWKS 기반 RS256 서명 검증**(+ issuer/audience 검사, OAuth `state`, public client PKCE — 클라이언트 시크릿 제거)으로 강화한다. v2에서만 "decode-only, 엣지 신뢰" 모델이 비로소 타당하다. v1 운영자는 v2 배포 전까지 엣지 인증을 **exp 전용**으로 간주해야 한다.
