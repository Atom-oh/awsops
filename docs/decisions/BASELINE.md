...
> 범위 = **v2 현행 진실.** v1(`src/`, CDK/EC2/Steampipe, `/awsops` basePath)은 별도 레거시 프로덕션 — 이 문서의 제약 대상이 아니다(§1 범위 참조).
...
- **범위 = v2.** v1(`src/`)은 별도 레거시 — v1 패치는 이 BASELINE의 "현행 진실 위반"이 아니다. v1 규칙은 `src/**/CLAUDE.md`.
...
| **GATED** | 운영 자가치유: 호스트 자기 서비스 재배포(Aurora secret 회전 복구) | `secret_rotation_redeploy_enabled` | EventBridge(RotationSucceeded)→Lambda→`ecs:UpdateService` force-new-deployment **자기 web 서비스 한정**. IAM 1 ARN·secret-id fail-closed·default-off. **NOT** AWS-리소스 변경, ADR-005 frozen substrate **아님**(별개 범주). CloudTrail trail 의존 | ADR-015 |
...
> 통합 ADR 14개. 상세·근거는 각 ADR. (옛 46개 → `../history/ADR-MAPPING.md`, 본문은 git tag `adr-legacy-2026-06-22`.)
