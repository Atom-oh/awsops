...
// ═════════════════════════ 8. FEATURE MAP ═════════════════════════
{
  ...
  addStatBand(s, [
    ...
    { value: "Cmd-K", label: "전 페이지·리소스 즉시 검색 팔레트" },
  ]);
  addFooter(s, ++pageNum);
  s.addNotes(`[요약]
...
// ═════════════════════════ 11. PRIVATE EDGE ═════════════════════════
{
  ...
  const pts = [
    ["엣지에서 끝나는 인증", "모든 요청은 Lambda@Edge가 RS256 서명·발급자·만료를 검증. 통과 못 하면 오리진에 닿기도 전에 로그인으로 회송 — 대시보드 서버는 인증된 트래픽만 봄"],
    ["표준 기반 로그인", "Cognito User Pool + PKCE 공개 클라이언트(시크릿 없음). 세션 토큰 12시간, 서명 검증은 JWKS 공개키 기반"],
...
// ═════════════════════════ 16. VALUE 1: UNIFIED VISIBILITY ═════════════════════════
{
  ...
  s.addText(
    ...
    "• ClickHouse 트레이스가 서비스 맵(호출 그래프)을 채우고, Prometheus 메트릭이 AI 심층 진단의 근거로 흡수\n" +
...
// ═════════════════════════ 17. VALUE 2: RCA ═════════════════════════
{
  ...
  const cols = [
    ...
    ["③ AI로 종합한다", "그래프에서 'AI에 질문'을 누르면 챗이 해당 리소스 컨텍스트를 물고 열림. 인시던트 섹션은 알람과 변경 이력을 자동 수집해 원인 후보를 서술 — 신호 조합을 AI가 대신", C.subtitleOrange],
  ];
...
// ═════════════════════════ 18. VALUE 3: COST ═════════════════════════
{
  ...
  const nodes = [
    ...
    ["④ 절감", "AI Idle Scan이 절감액 추정 달린 목록 생성", C.openaiGreen],
  ];
