<!-- generated-by: co-agent · source: CLAUDE.md · claude-md-sha: 0f7bb72f6c67 · generated-at: 2026-08-12 · DO NOT EDIT — edit CLAUDE.md then run /co-agent sync-context -->
...
- Flag-gated Steampipe Fargate+Lambda inventory sync (default off) is the only sanctioned Steampipe in v2 — a batch loader into Aurora, not a live-query service. The `aws-data` chat route and the 6 auto-collect collectors **keep running as routes** — only their **Steampipe execution step is HARD-DISABLED**: `steampipeAvailable()` (`web/lib/aws-data.ts`) returns `false` unconditionally because ADR-001/010 prohibit any live-Steampipe query path in v2 (live AWS reads go through AgentCore MCP Lambda tools), and all 7 callers fail-open to normal chat routing on that `false` (see Chat routing above) — the route/collector isn't dead, only the Steampipe step inside it is skipped. The SELECT-only guard, 200-row cap and `SQL_GEN_PROMPT` are retained **dark code** — do not flag them as dead, and do not "fix" `steampipeAvailable()` back onto `steampipe_enabled` (that flag toggles only the batch inventory-sync worker; gating on it re-opens the prohibited path).
