...
- Home-dashboard trend quick wins (v1 parity): the resource-trend chart gains show/hide toggle chips grouped as Core Resources (top 5, visible by default) and Other Resources (default-hidden) — line colors stay pinned when toggling — and an inline summary KPI bar shows tracked resource types · total resources · 7d net change (±-colored; '—' when fewer than two snapshots or none lands within the ±2-day tolerance, never a fabricated 0).
...
- 홈 대시보드 추세 퀵윈(v1 패리티): 리소스 추세 차트에 Core Resources(상위 5종, 기본 표시)/Other Resources(기본 숨김) 그룹의 시리즈 토글 칩 추가(토글해도 라인 색상은 고정), 인라인 요약 KPI 바(추적 리소스 타입 수 · 전체 리소스 · 7일 순증감 ± 색상 — 스냅샷 2개 미만이거나 ±2일 허용 범위 내 스냅샷이 없으면 0을 지어내지 않고 '—') 추가.
