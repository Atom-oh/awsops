...
- Inventory quick wins: the CloudFront table gains a Name column (tag-derived), the CloudTrail table gains a Last Delivery column and its detail panel gains the CloudWatch Logs / digest delivery timestamps-and-errors plus stop-logging time (visible after the next sync run), and the ECR table gains an Encryption column (AES256/KMS).
...
- 인벤토리 퀵윈: CloudFront 테이블에 Name 컬럼(태그 파생), CloudTrail 테이블에 Last Delivery 컬럼 + 상세 패널에 CloudWatch Logs/다이제스트 배달 시각·오류와 로깅 중지 시각(다음 sync 실행 후 표시), ECR 테이블에 Encryption 컬럼(AES256/KMS) 추가.
