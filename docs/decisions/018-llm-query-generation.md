...
4. **비용 예산**: 인스턴스당 **ISO 주 3회**, 그리고 **연속 3주** 소진되면 스키마가 바뀔 때까지 정지
   ...
   컬럼과 완전히 분리된 전용 북키핑 행**(`__diag_signal_budget__`, `meta.budget` 필드)에만 저장한다 — 처음엔
   콘텐츠와 같은 컬럼을 공유했는데, 예산의 정체성을 보존하려던 수정(오래된 마커 유지, 특정 키를 일치검사에서
   제외)이 매번 콘텐츠 행도 같이 잘못된 버전으로 태깅해서, 스키마가 그 오래된 태그가 실제로 가리키던 스키마로
   **롤백**되면 일치검사가 거짓으로 매칭돼 새로 만든(잘못 태깅된) 콘텐츠를 옛 스키마의 것처럼 그대로 서빙하는
   결함으로 이어졌다(review, 이번 라운드). 컬럼을 분리한 뒤로 콘텐츠는 항상 **현재** 스키마의 진짜 버전만
   가진다. 아직 해소되지 않은(active retry 또는 이번 주 소진) 경우에만 이 북키핑 행을 남기고, ready 로
   정산됐거나 플래그가 꺼진(DISABLED) 경우는 아예 남기지 않아 다음 실행이 재생성을 시도하지 않는다(review
   MAJOR: 이전 설계는 ready 인 경우까지 마커를 남겨 스키마 불변에도 매주 Bedrock 을 재호출했다). 마커는
   스키마 해시와 독립적으로 읽어 churn이
   예산을 리셋하지 못하게 하고, **연속 3주 park**(스키마 무관)은 스키마가 실제로 바뀌면 해제한다 — 단, 이 해제는
   **주 경계에서만** 일어난다(그 주에 원래도 예산이 갱신되는 시점이므로 추가 예산이 아니다). **같은 주 안에서
   스키마가 바뀌는 것은 예산을 갱신하지 않는다** — 인스턴스당 주 3회는 스키마별이 아니라 **인스턴스당 하드
   상한**이라, 같은 주에 스키마가 N번 "정말로" 바뀌면 3×N회를 허용하는 셀 것이므로(리뷰에서 실제로 시도해
   회귀 확인함) 그 주에 예산이 소진된 인스턴스는 스키마가 바뀌어도 다음 주까지 기다린다. 플래그가 꺼진 기간은
   과금하지 않는다. **v4→v5 전환**: 이 북키핑 행은 v5 부터 존재하므로, v5 이전에 이미 park/capped 였던
   인스턴스는 배포 직후 그 전용 행이 없어 "예산 없음"으로 읽혀 하드 상한을 위반하는 무료 재시작을 받을 수
   있었다(리뷰, 1차) — `datasource_index.py`가 예산 행이 비어있고 콘텐츠의 저장된 `schema_version` 자체가
   예전 마커 문자열이면 부트스트랩한다. 다만 **옛 해시를 그대로 유지하면 안 되고**(v4 해시는 v5 해시와 절대
   같을 수 없어, 스키마 불변에도 주 경계 언파크 체크가 "진짜 바뀜"으로 오판 — 리뷰 2차), **현재 버전으로
   재고정해서도 안 된다**(그러면 이 부트스트랩 시점에 스키마가 실제로 바뀌었더라도 옛 캡에 영구 흡수되는데,
   v4 해시와 v5 해시는 어느 방향으로도 비교 불가능해 구분할 방법이 없다 — 리뷰 3차). 그래서 부트스트랩은
   **연속소진주차수(streak)만** 이어받는다. attempts 는 옛 마커의 주차가 **현재 주와 같을 때만** 그 값을
   이어받고(배포가 주 중간에 일어나면 v4 가 이미 그 주의 예산 일부를 썼으므로, 무조건 0으로 리셋하면 같은 주에
   v4 소비분 + v5 fresh 예산을 합쳐 하드 상한을 넘길 수 있다 — 리뷰 4차), 옛 마커가 지난 주 것이면 0으로
   시작한다(주 경계가 자연스럽게 예산을 갱신하는 것과 동일). 이번 배포로 이전에 capped 였던 인스턴스마다
   최대 주 1회분(제한적, 상한 있음) 의 재평가 비용이 들지만, 비교 불가능한 해시 알고리즘 전환에서 스키마를
   잘못 귀속시키지 않는 대가로는 합리적이다.
