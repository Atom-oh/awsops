...
## Role
89 client components across 11 subdirectories: `ui` (shared primitives), `shell` (AppShell, Sidebar, LanguageProvider, AccountSelector, etc.), `charts`, `chat`, `inventory` (+`metrics/`), `eks`, `diagnosis`, `datasources`, `insights`, `nfm`, `overview`.
...
- `inventory/metrics/MetricTable.tsx` — a declarative `MetricCol` model (`{value, render?, danger?, facet?, type}`) gets you sort, global search, facet filters, and a "problems only" toggle for free. Per-service tables (Ec2/Rds/Alb/...) are written purely as column definitions. Opt-in, backward-compatible props: `facetValues` (multi-value facets — element-level matching, since exact-match on the joined display value misses composite rows), `maxRender`+`capKeep` (a render-stage row cap — search/sort/counters still operate on the full set, so a data-stage cut that silently zeroes out an exact search is avoided), `rowClass` (per-row class hook, e.g. idle-row dimming).
