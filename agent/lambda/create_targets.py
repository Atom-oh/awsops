...
if gw:
    ...
    create_target(gw, 'rds-mcp-target', 'awsops-rds-mcp',
        ...
        [{'name': n, 'description': d, 'inputSchema': s} for n, d, s in [
            ...
            ('execute_sql', 'SQL via Data API (SELECT only; PostgreSQL targets only — MySQL/MariaDB unsupported)', {'type': 'object', 'properties': {'sql': prop('string', 'SQL'), 'resource_arn': prop('string', 'ARN')}, 'required': ['sql', 'resource_arn']}),
