...
- Chart form-fit pass (dataviz methodology): two close-magnitude comparisons trade donuts for ranked horizontal bars — the home 'EC2 instance types' breakdown (nominal categories with many close values — exact ranking beats slice-angle comparison; the summary API's top-10 cap is now disclosed in the card subtitle, where the donut's Other rollup used to hint at it) and Bedrock's 'Cost per model' (sharing ONE row order with its sibling invocation bars so a model sits on the same row in both — one comparable block). Genuinely part-to-whole donuts (categories, pod status, service types, severity, job status) are deliberately kept. Colorblind safety of the new diverging pair (see the amended cost-impact entry) was validated (CVD ΔE 13.9 light / 12.2 dark, both ≥ the 8 target); every bar carries a visible signed value label.
...
- Prometheus/Mimir AI query generation is now exact-instance grounded: Korean resource concepts prioritize relevant metrics, at most two metric-metadata candidates use 3-second upstream deadlines, and every metric referenced by generated PromQL is live-confirmed before execution. Each confirmed candidate is then executed once with a read-only 5-second bound for syntax/type validation; one conclusive failure gets a single AI correction, while missing grounding or transient connector failures return 503. PromQL generation now requires an instance id; other legacy slug DSL paths remain.

- Datasource detail pages gain a pre-built card dashboard: registering a datasource (and each daily index run) derives an expected card set from the cached schema — the queries each card uses are stored ahead of time (new `datasource_dashboard_cards` table + a deterministic `card_catalog` in the index worker; prometheus/mimir 13 cards covering targets, CPU, memory, disk, load, network, containers, and restarts; loki 2, tempo 2, clickhouse 2) — ready Prometheus/Mimir cards are live-validated against the exact datasource before registration (a conclusive PromQL error disables only that card; a transient connector failure preserves the previous card set) — card building runs inside the existing datasource-index job, so it is gated on `datasource_diagnosis_enabled` (default false; requires `workers_enabled`/`agentcore_enabled`/`integrations_enabled`) like the diag-signal chips — and the page executes the stored queries live on view through the existing read-only query API (stat/timeseries/table cards; unavailable cards render dimmed with what's missing; a failed card shows an inline error, never a silent zero).
...
- 차트 형태 적합화(dataviz 방법론): 근접 크기 비교 2곳을 도넛 대신 순위 가로 바로 — 홈 'EC2 인스턴스 유형'(명목 카테고리의 다수 근접값 — 조각 각도보다 정확한 순위 비교; summary API의 상위 10개 캡을 카드 부제에 공지 — 종전 도넛의 기타 롤업이 암시하던 부분성을 명시화)과 Bedrock '모델별 비용'(형제 호출 수 바와 동일한 행 순서를 공유해 같은 모델이 두 차트의 같은 행에 위치 — 한 블록으로 비교). 진짜 part-to-whole인 도넛(카테고리·파드 상태·서비스 타입·심각도·작업 상태)은 의도적으로 유지. 새 diverging 색상쌍(비용 영향 항목에 반영)의 색각 안전성 검증 완료(CVD ΔE 라이트 13.9/다크 12.2, 목표 8 이상), 모든 바에 서명된 값 라벨 표시.
...
- Prometheus/Mimir AI 쿼리 생성은 이제 정확한 인스턴스에 기반해 닫힌 방식으로 동작: 한국어 리소스 개념으로 관련 메트릭을 우선하고, 최대 2개 메트릭 메타데이터 후보를 업스트림 3초 제한으로 조회하며, 생성 PromQL이 참조한 모든 메트릭을 실행 전에 라이브 확인. 확인된 후보는 read-only 5초 제한으로 1회 실행해 문법/타입을 검증하고 확정 오류는 AI 교정 1회만 허용; grounding 부재나 일시적 커넥터 실패는 503. PromQL 생성은 인스턴스 id가 필수이며 다른 레거시 slug DSL 경로는 유지.

- 데이터소스 상세 페이지에 사전 생성 카드 대시보드 추가: 등록 시(및 일일 인덱스 배치마다) 캐시된 스키마로부터 예상 카드 세트를 도출하고 각 카드가 사용할 쿼리를 미리 저장(신규 `datasource_dashboard_cards` 테이블 + 인덱스 워커의 결정론적 `card_catalog` — prometheus/mimir는 타깃·CPU·메모리·디스크·로드·네트워크·컨테이너·재시작을 포괄하는 13종, loki 2종·tempo 2종·clickhouse 2종) — ready Prometheus/Mimir 카드는 등록 전에 해당 데이터소스에서 라이브 검증하며(PromQL 문법 오류는 해당 카드만 비활성화, 일시적 커넥터 장애는 기존 카드 세트 보존) — 카드 빌드는 기존 datasource-index 잡 내부에서 실행되므로 diag-signal 칩과 동일하게 `datasource_diagnosis_enabled` 게이트(기본 false; `workers_enabled`/`agentcore_enabled`/`integrations_enabled` 선행) 하에 동작 — 페이지가 저장된 쿼리를 기존 read-only 쿼리 API로 조회 시점에 라이브 실행해 렌더링(stat/시계열/테이블 카드, 미충족 카드는 누락 항목과 함께 비활성 표시, 실패 카드는 조용한 0이 아니라 인라인 에러로 표시).
...
- **통합**
  - 외부 데이터소스 8종: Prometheus, Mimir, Loki, Tempo, ClickHouse, Jaeger, Dynatrace, Datadog — 인스턴스 CRUD + 크리덴셜 저장(admin), SSRF 가드 연결 테스트, kind별 기본 인스턴스 지정, read-only 쿼리 실행, 자연어 쿼리 초안 생성(Prometheus/Mimir는 검토 전 bounded read-only 검증 1회, 나머지는 초안 전용), 사전 정의 진단 시그널.
