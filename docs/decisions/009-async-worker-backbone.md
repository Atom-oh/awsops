...
- **쓰기 키 = immutable `sub`(단일)** — 사용자 경로 enqueue 는 전부 `user.sub` 를 기록한다
  ...
  admin CRUD 이며 `created_by` 에 같은 키를 쓴다. `report_schedules` 와 그 dispatcher 도 sub 다. 단 **`created_by` 계열(`skills`·`agents`·`action_plans`·`architecture_intent`)은 이 컷오버 대상이 아니다** — 그 writer 들이 의도적으로 email 을 쓰는 admin 귀속 값이라 재작성해도 새 행이 다시 email 이 된다. 대신 그 값을 **부정 판정**에 쓰는 곳(4-eyes 자기승인 검사)은 email·sub 를 **집합으로** 비교해야 한다: 한 문자열끼리 비교하면 같은 사람의 두 표기가 서로 달라 자기 plan 을 승인할 수 있고, 증명 불가한 조합(email-keyed 생성자 + verified email 없는 승인자)은 거부한다(`identityKeys()`, flag 비의존). **사용자당 활성 스케줄 1개는 이제 DB 가 강제한다** (partial unique index `uq_schedule_one_active`, migration 01KZ3C7Q…) — `upsertSchedule()` 안에만 있던 불변식이라 backfill 이 다른 주기의 활성 행 위로 소유자를 옮겨도 아무 제약이 걸리지 않았다. 트랜잭션 내 검사도 시도했지만 PG 17 실측에서 SERIALIZABLE 스냅샷이 동시 커밋을 가려 검사를 통과시켰다 — 읽기 검사로는 막을 수 없다는 신호였고, 그래서 인덱스로 옮겼다. lineage 서브쿼리가 `worker_jobs` 를 JOIN 해 `payload->>'account'` 로 좁히므로, account 키가 없는 구세대 payload 행은 baseline 후보에서 빠진다 — 배포 직후 1회 NULL parent 가 나올 수 있고, 이는 account 스코핑의 의도된 대가다(review MINOR).
