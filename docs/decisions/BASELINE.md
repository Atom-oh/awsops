...
| GATED operator CI access | `github_actions_enabled` | Default off; exact origin/production OIDC trust, web-only ECR/ECS release permissions, explicitly scoped migration-secret reads, and verifier-secret metadata. No product mutation authority or broad infrastructure-apply role. | ADR-005 operator boundary; [access contract](../reference/github-actions-access.md) |
