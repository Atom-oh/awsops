...
- [ ] Implement `agent/lambda/notion_mcp.py`: `lambda_handler(event, context)` parsing
  `tool_name`/`arguments` like `network_mcp.py`; `_get_token()` (Secrets Manager
  `ops/awsops-v2/integrations/notion`, env `NOTION_SECRET_NAME` overridable, module-cached);
  `_notion_request(method, path, body=None)` via stdlib `urllib.request` (timeout, JSON);
  `_clamp_page_size()`; the 3 tool functions; `ok()`/`err()` helpers mirroring `network_mcp.py`.
  Stdlib + `boto3` only (no third-party HTTP lib — zip-packaging constraint).
...
- [ ] Run `cd scripts/v2/agentcore && python3 catalog_check.py` → prints `OK` and lists
  `notion-mcp` among the lambda_keys (cross-checks ai.tf).
- [ ] Commit: `feat(agent-platform): register notion-mcp-target on external-obs gateway`.

### Task 3: provision the Notion Lambda + secret + IAM (flag-gated)
**Files:**
...
- [ ] Restructure `local.agent_lambdas` to `merge(<existing agentcore_enabled ? {...} : {}>,
  ...
  "notion_mcp.lambda_handler" } } : {})` — so the Notion Lambda exists only when
  `agentcore_enabled && integrations_enabled`. (`lambda_arns` output auto-includes it.)
- [ ] Add `aws_secretsmanager_secret "notion"` (`count = local.integ_count`,
  `name = "ops/${var.project}/integrations/notion"`, optional KMS = the inc2 integrations CMK if
  present; value injected out-of-band — NO `aws_secretsmanager_secret_version` with a literal).
- [ ] Grant the shared `agent_lambda` exec role `secretsmanager:GetSecretValue` scoped to the
  notion secret ARN (`count = local.integ_count`; reuse the `ops/${var.project}/integrations/*`
  pattern). No `0.0.0.0/0`, no `Principal:"*"`, no wildcard resource beyond the scoped prefix.
...
- [ ] Commit: `feat(agent-platform): provision notion-mcp Lambda + secret + IAM (integrations_enabled-gated)`.
