...
// never a silent success. This is a notification to the app's own topic (ADR-040/041 governed
// external-comms), not an AWS-resource mutation.
export const dynamic = 'force-dynamic';

export async function POST(request: Request) {
  const user = await verifyUser(request.headers.get('cookie'));
  if (!user) return NextResponse.json({ message: 'unauthenticated' }, { status: 401 });
  if (!isAdmin(user)) return NextResponse.json({ message: 'admin only' }, { status: 403 });
  const arn = topicArn();
  ...
  try {
    ...
    return NextResponse.json({ message: `publish failed: ${String(e)}` }, { status: 502 });
  }
}
