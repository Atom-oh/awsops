...
describe('ServiceNetworkTopology', () => {
  it('preserves partial, stale and retained snapshot status with per-source reasons', async () => {
    serve({ service: () => json({ ...snapshot, collection: {
      ...
      attempted_at: '2026-09-11T12:00:00Z', captured_at: snapshot.captured_at,
      sources: [{ sourceId: 'tempo:fixture', status: 'error', reasons: ['timeout'], itemCount: 0 }],
    } }) });
    ...
    expect(alert.textContent).toMatch(/부분 수집/);
    expect(alert.textContent).toMatch(/오래된 데이터/);
    expect(alert.textContent).toMatch(/이전 그래프/);
    expect(alert.textContent).toContain('tempo:fixture');
  ...
  it.each(['class', 'account'])('rejects a snapshot missing its %s scope', async field => {
    serve({ service: () => json({ ...snapshot, [field]: undefined }) });
    render(<ServiceNetworkTopology {...props} />);
  ...
  it('preserves evidence filters through a query and its result', async () => {
    serve();
    render(<ServiceNetworkTopology {...props} />);
    fireEvent.click(screen.getByRole('checkbox', { name: '서비스 관측' }));
    fireEvent.click(await ready());
    expect((screen.getByRole('checkbox', { name: '서비스 관측' }) as HTMLInputElement).checked).toBe(false);
  ...
  it('accepts empty source arrays as unavailable observations, never as proof of zero traffic', async () => {
    serve({
      ...
      service: () => json({ ...snapshot, nodes: [], edges: [], captured_at: null, collection: { status: 'empty', stale: false } }),
    });
    render(<ServiceNetworkTopology {...props} />);
