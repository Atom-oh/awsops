...
- v1 Steampipe queries: JSONB column names must be verified against the real schema (e.g. MSK `provisioned`, OpenSearch `encryption_at_rest_options`); SCP-blocked columns (`mfa_enabled`, `attached_policy_arns`, Lambda `tags`) can't appear in list queries; no `$` in SQL (`conditions::text LIKE '%..%'` instead); list queries need an `account_id` column. (ADR-010 2026-09-02 개정: 명시적 위험 수용 시 예외 — 현재 iam_role.attached_policy_arns 수용됨)
