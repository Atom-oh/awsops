...
def _error_code(error):
    """Fixed gateway diagnostics; never include raw AWS/SDK exception text."""
    ...
    codes = {
        ...
        "UnauthorizedException": "aws_access_denied",
        "ExpiredToken": "aws_credentials_expired",
        "ExpiredTokenException": "aws_credentials_expired",
        "InvalidClientTokenId": "aws_credentials_expired",
    ...
    for key in catalog.GATEWAYS:
        ...
        if name in existing:
            ...
            try:
                ...
                log(f"gateway:{key}", failure_status, _error_code(error))
            continue
        try:
            ...
            log(f"gateway:{key}", "ERR", _error_code(error))
    return ids
...
def ensure_targets(ctrl, ac, gw_ids, skip_names=frozenset()):
    ...
    for tname, spec in catalog.TARGETS.items():
        ...
        try:
            ...
            log(f"target:{tname}", "ERR", _error_code(error))
