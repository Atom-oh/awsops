...
>
> **Follow-up (2026-08-05, ADR-017 재개정 — anti-drift 정합화):** 위 amendment가 "재-ack 없이 흡수 — 알고서 수용한 잔여 리스크"라고 기록한 사유는 ADR-017의 2026-08-05 재개정으로 **해소**됐다 — 남은 벤더 호스팅 3종(Datadog·Dynatrace·New Relic, 자체 호스팅 5종은 카탈로그에서 제거됨)에 런타임 fail-closed 툴 allowlist(`OFFICIAL_MCP_TOOL_ALLOWLIST_JSON` → `agent.py`가 교집합, 공란/env부재 ⇒ 툴 0개)가 코드로 강제되어 벤더의 write 툴 추가가 더 이상 무심사로 흡수되지 않는다. **이 문단의 나머지(SSRF/DNS-rebinding/private-CIDR가 managed-egress 경로에서 코드로 강제 불가능하다는 것)는 그대로 유효하다** — allowlist는 어떤 툴이 노출되는지를 제한할 뿐, 그 툴 호출이 어디로 나가는지의 connect-time 검증을 대신하지 않는다. `official_mcp_enabled`는 일반 GATED(BASELINE §2). ClickHouse의 stdio 내장(`CLICKHOUSE_OFFICIAL_MCP`)은 이 리스크와 무관한 별도 메커니즘이며 별도 사유로 여전히 do-not-enable/FROZEN이다(자체 람다의 SSRF/read-only 1차 방어를 대체 없이 상실 — ADR-017 §Trade-offs, BASELINE §2).
