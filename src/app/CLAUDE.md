...
- `alert-settings/page.tsx` — 알림 파이프라인 설정 (웹훅 URL, HMAC 시크릿, Slack 채널 라우팅, admin 전용)
- `event-scaling/page.tsx` — 이벤트 사전 스케일링 (ADR-010 Phase 1+2, 이벤트 등록 + 과거 메트릭 분석 + AI 플랜 + 스크립트 다운로드, admin 전용, 실행 없음)
...
### External Datasources (2)
- `datasources/page.tsx` — 외부 데이터소스 CRUD (Prometheus, Loki, Tempo, ClickHouse, Jaeger, Dynatrace, Datadog)
- `datasources/explore/page.tsx` — 쿼리 콘솔 (자연어 → PromQL/LogQL/TraceQL/SQL, 결과 테이블/차트)
...
## API 라우트 (20개) — 상세: `src/app/api/CLAUDE.md`

| API | 설명 |
|-----|------|
| `api/parity/route.ts` | ADR-030 dual-write parity 체크 (JSON vs Aurora 카운트) |
| `api/ai/route.ts` | AI 라우팅 (11 routes incl. datasource, 멀티 라우트, SSE, 도구 추론, 토큰 추적) |
...
| `api/k8s/route.ts` | EKS kubeconfig 등록 (Access Entry) |
| `api/report/route.ts` | AI 종합 진단 리포트 생성 · S3 저장 · 스케줄링 |
| `api/alert-webhook/route.ts` | 알림 웹훅 수신 (CloudWatch SNS / Alertmanager / Grafana / Generic) + HMAC + 상관 분석 |
| `api/notification/route.ts` | Slack/SNS 알림 발송 (Block Kit, 심각도 채널 라우팅) |
| `api/event-scaling/route.ts` | 이벤트 사전 스케일링 (CRUD + 메트릭 + Bedrock 플랜 + 스크립트, admin 전용, 실행 없음) |
...
- `alert-settings/page.tsx` — Alert pipeline settings (webhook URL, HMAC secret, Slack channel routing, admin-only)
- `event-scaling/page.tsx` — Event pre-scaling (ADR-010 Phase 1+2, event registration + historical metrics + AI plan + script download, admin-only, no execution)
...
### External Datasources (2)
- `datasources/page.tsx` — Datasource CRUD (Prometheus, Loki, Tempo, ClickHouse, Jaeger, Dynatrace, Datadog)
- `datasources/explore/page.tsx` — Query console (natural language → PromQL/LogQL/TraceQL/SQL, result tables/charts)
...
## API Routes (20) — details in `src/app/api/CLAUDE.md`

| API | Description |
|-----|------------|
| `api/parity/route.ts` | ADR-030 dual-write parity check (JSON vs Aurora counts) |
| `api/ai/route.ts` | AI routing (11 routes incl. datasource, multi-route, SSE, tool inference, token tracking) |
...
| `api/k8s/route.ts` | EKS kubeconfig registration (Access Entry) |
| `api/report/route.ts` | AI diagnosis report generation · S3 storage · scheduling |
| `api/alert-webhook/route.ts` | Alert webhook ingestion (CloudWatch SNS / Alertmanager / Grafana / Generic) + HMAC + correlation |
| `api/notification/route.ts` | Slack/SNS notification dispatch (Block Kit, severity-based channel routing) |
| `api/event-scaling/route.ts` | Event pre-scaling (CRUD + metrics + Bedrock plan + script download, admin-only, no execution) |
