...
### Post-acceptance corrections / 채택 후 정정 (2026-06-03, co-agent 3-AI review)

- **Direct-to-ALB ingress needs an explicit SG rule (reconcile with ADR-028/ADR-020).** The Context says VPC-internal emitters "reach the ALB directly, bypassing CloudFront," but ADR-028 and ADR-020 set the **ALB security-group ingress to the CloudFront origin prefix list only**. As written those are contradictory: a VPC-internal emitter's source IP is not in the CloudFront prefix list, so the direct path is blocked. The webhook design requires an **additional ALB SG ingress rule** for the emitter subnets/CIDR (Alertmanager/Grafana/poller). Relatedly, the rate-limit client-IP extraction assumes a fixed **CloudFront+ALB = 2-hop** `x-forwarded-for`; a request that reaches the ALB **directly is 1 hop**, so the "second-to-last XFF entry" logic mis-identifies the client IP on the direct path and must branch on ingress path. / **ALB 직접 인그레스에는 명시적 SG 규칙 필요(ADR-028/ADR-020과 정합).** Context는 VPC 내부 발신기가 "CloudFront 우회해 ALB로 직접" 접근한다고 하나, ADR-028·ADR-020은 **ALB 보안그룹 인그레스를 CloudFront origin prefix list 전용**으로 둔다. 그대로면 모순 — 내부 발신기 IP는 prefix list에 없어 직접 경로가 막힌다. 발신기 서브넷/CIDR(Alertmanager/Grafana/poller)에 대한 **추가 ALB SG 인그레스 규칙**이 필요하다. 또한 rate-limit 클라이언트 IP 추출은 **CloudFront+ALB = 2-hop** XFF를 가정하나, ALB 직접 도달은 **1-hop**이라 "끝에서 두 번째 XFF" 로직이 직접 경로에서 클라이언트 IP를 오인하므로 인그레스 경로별 분기가 필요하다.
- **`cloudwatch` / `sqs` are not HMAC sources.** AWS-native CloudWatch (delivered via SNS) and SQS **cannot compute a customer-keyed `HMAC_SHA256(secret, rawBody)`** — they do not sign payloads with an arbitrary shared secret. Those two entries in the per-source secret map are therefore not authenticated by HMAC; CloudWatch arrives via **SNS subscription confirmation + the ADR-012 SNS→SQS→IAM polling path**. HMAC applies only to the **push emitters** that can be configured to sign — `alertmanager`, `grafana`, `generic`. The source list should document this split rather than imply all five sources share the HMAC scheme. / **`cloudwatch`·`sqs`는 HMAC 소스가 아니다.** AWS 네이티브 CloudWatch(SNS 전달)와 SQS는 **고객 시크릿으로 `HMAC_SHA256(secret, rawBody)`를 계산할 수 없다** — 임의 공유 시크릿으로 페이로드를 서명하지 않는다. 따라서 소스별 시크릿 맵의 이 두 항목은 HMAC으로 인증되지 않으며, CloudWatch는 **SNS 구독 확인 + ADR-012 SNS→SQS→IAM 폴링 경로**로 도착한다. HMAC은 서명 설정이 가능한 **push 발신기**(`alertmanager`·`grafana`·`generic`)에만 적용된다. 소스 목록은 다섯 소스가 모두 HMAC을 공유하는 듯한 서술 대신 이 구분을 명시해야 한다.
