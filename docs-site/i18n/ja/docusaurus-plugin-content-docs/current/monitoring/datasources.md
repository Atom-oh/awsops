...
| 設定 | デフォルト | 説明 |
|------|--------|------|
| **Timeout** | 10 秒 | アップストリームのクエリ実行上限（秒、1–60）。ClickHouse はすべての経路（Explore・サービスグラフ・エージェント）に適用され、コネクタは自身の HTTP タイムアウトをその上に揃えます。Prometheus/Mimir は Explore 経路の API `timeout` パラメータとして適用され、コネクタの 12 秒 HTTP タイムアウトの下で 10 秒にキャップされます |
