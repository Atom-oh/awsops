...
## Rules
- Auth: private APIs call `verifyUser(request.headers.get('cookie'))` (`lib/auth.ts`, re-verifies the `awsops_token` cookie via RS256 JWKS) → 401 if null. Admin-only routes additionally check `isAdmin()` (`lib/admin.ts`). This is BFF-level authorization, distinct from the edge's authentication allowlist (root CLAUDE.md's public-path list — `/api/health`, `/api/auth/signout`, `/login`, `/api/auth/login`, `/icon.svg`, `/_next/static/*`, and the ADR-013 `/api/incidents/webhook` carve-out): a route being edge-public does not mean it skips `verifyUser()` (`/api/health` happens to need neither), and every other data-returning or billable route must call `verifyUser()` regardless of the edge allowlist.
