...
- **Phase 2 (Aurora durable token budget)** — implemented **in v1 (`src/lib/ai-cost/token-budget.ts` + `src/lib/db/token-budget-writer.ts`, Aurora dual-write+hydrate)**: `ai_token_budget` table + dual-write + cold-start hydrate (the in-process budget no longer resets on restart). See `docs/superpowers/plans/2026-06-09-adr-033-phase2-durable-budget.md`. ⚠️ **The v2 `web/` chat path (`web/app/api/chat/route.ts`) has NO budget gate yet — the v2 port is an open follow-up.** / **v2 `web/` 챗 경로에는 아직 예산 게이트가 없음** — v2 포팅은 미착수 후속 작업.
