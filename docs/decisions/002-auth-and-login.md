...
> **Amendment (2026-08-19, PWA install):** `is_public()`'s allowlist grew by 5 exact-match static-asset paths — `/manifest.webmanifest`, `/apple-touch-icon.png`, `/icon-192.png`, `/icon-512.png`, `/icon-512-maskable.png`. iOS/iPadOS fetches the web app manifest and its icons to build the home-screen install prompt *without* sending the `awsops_token` cookie, so these paths must stay reachable unauthenticated the same way `/icon.svg` already was. Each entry is a static, parameter-free asset with no query-string or prefix widening (exact `uri in (...)` match), carries no session or account data, and there is no catch-all route for them to fall through to if the match fails — the same posture as the pre-existing favicon carve-out, just five more static files. `web/app/manifest.test.ts` pins each path against both the edge template and the actual `public/` files (3-way lockstep). Because `/manifest.webmanifest` is served by a route handler (`web/app/manifest.ts`) reachable unauthenticated and CDN-cacheable — a de-facto fourth `verifyUser()`-less handler alongside the three enumerated above — it must stay a static-constant route: no per-user, per-session, or environment-derived value may ever enter its response.
>
> **개정 (2026-08-19, PWA 설치):** `is_public()` 허용목록에 정적 자산 경로 5개(정확히 일치)가 추가됐다 — `/manifest.webmanifest`, `/apple-touch-icon.png`, `/icon-192.png`, `/icon-512.png`, `/icon-512-maskable.png`. iOS/iPadOS는 홈 화면 설치 프롬프트를 만들 때 `awsops_token` 쿠키 없이 web app manifest와 아이콘을 가져오므로, 기존 `/icon.svg`와 같은 이유로 미인증 상태에서도 도달 가능해야 한다. 각 항목은 쿼리스트링·prefix 확장이 없는 정적·파라미터 없는 자산(정확 일치 `uri in (...)`)이고 세션·계정 데이터를 담지 않으며, 매칭 실패 시 떨어질 catch-all 라우트도 없다 — 기존 파비콘 예외와 동일한 성격의 정적 파일 5개가 늘었을 뿐이다. `web/app/manifest.test.ts`가 엣지 템플릿과 실제 `public/` 파일을 3자 lockstep으로 고정한다. `/manifest.webmanifest`는 라우트 핸들러(`web/app/manifest.ts`)가 서빙하며 미인증·CDN 캐시 가능 상태로 도달 가능하다 — 위 세 개 열거 외의 사실상 네 번째 `verifyUser()` 미적용 핸들러이므로, 사용자별·세션별·환경별로 달라지는 값이 응답에 절대 들어가지 않는 정적-상수 라우트로 유지해야 한다.
