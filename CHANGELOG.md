...
- PR review pipeline: code-first review with bounded chair retries now pins the current target branch, or a verified two-parent merge boundary for a main-only audit of merged PRs. Publication rechecks HEAD, base and diff integrity; truncated or omitted content, missing responses and incomplete model findings cannot pass.
...
- PR 리뷰 파이프라인: 코드 우선 리뷰와 제한된 chair 재시도를 유지하며 현재 대상 브랜치 또는 이미 머지된 PR의 검증된 부모 커밋을 기준으로 고정합니다. 머지된 PR의 재검증은 main에서만 실행하며, 게시 전에 HEAD·기준 커밋·diff 무결성을 다시 확인합니다. 잘리거나 누락된 변경, 응답 누락, 미완료 모델 리뷰는 통과시키지 않습니다.
