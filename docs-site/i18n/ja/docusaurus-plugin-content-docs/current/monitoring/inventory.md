...
:::tip スナップショットベースのデータ
スナップショットは成功した（部分失敗でない）インベントリ sync の実行ごとにアカウント別で Aurora（`inventory_snapshots`）に記録されます（部分失敗の run は直前の正常値を保持するため、日次ポイントが欠けることがあります） — ダッシュボードのロードとは無関係で、参照時に追加の AWS API 呼び出しはありません。
:::
