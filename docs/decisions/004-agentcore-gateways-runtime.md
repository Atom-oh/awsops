...
Table access is an **explicit allowlist** (each table enumerated in a `GRANT SELECT`); no `ON ALL TABLES` and no `ALTER DEFAULT PRIVILEGES` blanket, so a table a future migration adds is invisible to this role until someone deliberately grants it. Because `execute_sql` is a model-invocable tool, credential-bearing columns are excluded via column-level `GRANT SELECT (…)` — notably `eks_registrations.auth` (a plaintext Kubernetes ServiceAccount bearer token the read APIs return as `mode` only) and `accounts.external_id` — which mirrors the application's redaction boundary at the DB level. The migration's `REVOKE EXECUTE … FROM PUBLIC` statements are **best-effort hardening**, not a guarantee: on RDS/Aurora those `pg_catalog` functions are owned by `rdsadmin`, so a revoke may fail (the migration `RAISE NOTICE`s and continues, and reports which targeted functions still have PUBLIC EXECUTE afterwards). The boundary is the role attributes above, and that argument holds regardless of whether the revokes land: `query_to_xml` executed *as this role* can only reach what this role can reach, and `pg_cancel_backend` can only signal same-role sessions.
