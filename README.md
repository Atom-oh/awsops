...
| `diag_signal_querygen_enabled` | LLM fallback for ONE Explore diag-signal chip, only when a kind's deterministic catalog yields **zero ready rows** (a partial match is not topped up), and only for the chips — the diagnosis report never uses generated rows. Separate from `graph_querygen_enabled`; both need `datasource_diagnosis_enabled` |
...
| `diag_signal_querygen_enabled` | Explore diag-signal 칩 **1개**의 LLM 폴백 — 그 kind의 결정론 카탈로그가 **ready 0행**일 때만 발동(부분 매칭은 보충하지 않음), 생성 행은 칩 전용(진단 리포트 미사용). `graph_querygen_enabled`와 **별개**, 둘 다 `datasource_diagnosis_enabled` 선행 |
