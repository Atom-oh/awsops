...
- Private CI migrations: an opt-in ARM64 Fargate executor applies the reviewed migration source over verified RDS TLS, using dedicated database-secret permissions inside the task. Image digest, task exit and nonce-bound evidence gate success; preview cannot authorize a release, and interrupted runs reconcile only their owned task.

- Deployment dependency readiness: authenticated probes verify the deployed web identity and fresh AgentCore SSM reads, then require nonce-bound evidence for curated inventory access, a known fresh CloudFront record and a bounded model call. The probe requires admin or deployment-verifiers and a process-wide cooldown. Collection summaries expose aggregate job scope and preserve missing/failed/unknown states, and disabled invocation/readiness/status runtime discovery does not query an unavailable SSM path. Status runtime discovery skips SSM client creation when the runtime parameter is explicitly empty, while preserving the legacy default when unset and explicit configured paths. Optional host-only inventory validates account scope, and inventory/worker images accept verified digests. AgentCore provisioning reconciles gateway roles and Lambda target ARNs/credentials while preserving deployed authentication and known routing IDs; typed gateway errors distinguish failed requests from readiness. Infrastructure activation and release verification remain explicit deployment steps.
