...
Do not infer idle resources from missing observations. Show each source's capture/query window and truncation. Keep all loaded data searchable before applying display limits (350 nodes, 700 edges). Drop dangling edges when bounding a view and show omitted counts. Reserve display priority for network connections, their endpoint/identity neighborhoods and service relations before unrelated inventory. Evidence filters also constrain search and selected details.
