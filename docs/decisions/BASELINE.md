...
| **LIVE** (외부 write 중 유일) | 진단 완료 SNS 이메일 통지 | `diagnosis_notify_enabled` | ON | **이미 켜져 있음** — IAM 단일 토픽 스코프, AWS-리소스 변경 아님(거버넌스 충족 — 아래 "주의" blockquote 참조) | ADR-013 |
...
| **GATED (승인됨·미구현)** | Network Path Check — 저장 가능한 비동기 read-only 경로 정책 점검 (`network_path` job) | `network_path_check_enabled` | **N/A** — terraform/variables.tf에 아직 없음(이 행은 spec 승인 기록이며 이 열의 "실제 조회로 확인한 상태" 요건을 만족시키는 라이브 OFF 조회가 아니다. Neptune 행의 N/A 관례를 따름) | `workers_enabled` 선행 예정. read-only 정적 분석만(SG/NACL/route/TGW/K8s policy/L7 describe) — Reachability Analyzer 생성·probe 실행 없음, `ec2:CreateNetworkInsightsPath`/`DeleteNetworkInsightsPath` 미부여 예정. 워커 role에 `sts:AssumeRole`→`AWSopsReadOnlyRole` 확장 예정(기존 웹/Steampipe/agent 패턴 재사용, 신규 신뢰 관계 아님) — **이 행은 spec의 Approved 전환만 반영하며, 플래그를 도입하는 구현 PR에서 실제 값으로 갈아 끼운다.** 근거는 ADR-019가 아니다 — ADR-019 §Decision은 이 spec을 "이 ADR에 영향받지 않음"이라고 명시한다 | — |
| **GATED(owner-override 예외, 승인됨·미구현)** | SG Rules & Usage — 일일 rule 인벤토리 + Athena 기반 90일 트래픽 증거 (`sg_rule_inventory`/`sg_rule_activity`/`sg_rule_source_validate` job) | `sg_rule_activity_enabled` | **N/A** — terraform/variables.tf에 아직 없음(위 행과 동일한 이유로 라이브 조회 대상 아님) | `workers_enabled` 선행 예정. **ADR-019가 등록하는 owner-override 예외는 정확히 역할 B(Athena/S3 write) 하나뿐이다** — 역할 A(rule 인벤토리, 기존 `AWSopsReadOnlyRole` 재사용, 웹/Steampipe/agent와 동일 패턴)는 예외가 필요 없다. 역할 B는 대상 계정의 **신규·격리된** `AWSopsSgRuleAthenaRole`을 assume — `AWSopsReadOnlyRole`은 건드리지 않으며, 공유 워커 task role은 이 role을 assume할 수 없다(전용 task role/definition 또는 broker Lambda로 격리, 구현 시 확정). 유효 조건(ADR-019 §3b, 셋 다 유지되어야 함): SELECT-only 생성 SQL, Glue mutating 동사 배제, S3 read/write가 서로 다른 두 위치로 스코프(Flow Log 소스=read만, 워크그룹 결과 prefix=read+write 둘 다 필요 — Athena 자신이 자기가 쓴 결과를 다시 읽음) + 신뢰 정책(ExternalId+principal 제한) + 전용/격리 결과 prefix + source/result 겹침 금지. `sg_rule_activity_max_query_bytes`(기본 100 GiB, 이 PR에서 spec에 신설)로 스캔 예산 제한. **이 행은 spec의 Approved 전환만 반영하며, 플래그를 도입하는 구현 PR에서 실제 값으로 갈아 끼운다.** | ADR-019 |
...
> 통합 ADR 19개. 상세·근거는 각 ADR. (옛 46개 → `../history/ADR-MAPPING.md`, 본문은 git tag `adr-legacy-2026-06-22`.)
...
| [019](019-athena-flow-log-query-classification.md) | SG Rules Athena 파이프라인 — ADR-005 owner-override 예외 (GATED, ADR-015 패턴) | Athena/Glue read-query(StartQueryExecution/GetQueryResults/StopQueryExecution 등)는 기존 CloudWatch Logs Insights 패턴(특히 `sg-analysis.ts`, 같은 데이터·같은 목적)과 동형이며 ADR-007 티어가 아니다. **역할 A**(기존 `AWSopsReadOnlyRole` 재사용, rule 인벤토리)는 예외가 필요 없다. **역할 B**(신규·격리된 `AWSopsSgRuleAthenaRole`, Athena/S3 write)는 이 저장소 최초의 write-capable cross-account 신뢰 관계이므로, "완화 불필요" 주장이 아니라 ADR-005에 대한 **좁고 명시적인 owner-override 예외**(ADR-015 패턴)로 등록한다 — §3b 세 통제가 유지되는 동안만 유효. `sg_rule_activity_enabled`가 이 결정에 따라 §2에 GATED(owner-override 예외)로 등록됨(승인됨·미구현). **`network_path_check_enabled`는 이 ADR의 범위 밖이다** — network-path spec은 이 ADR과 무관하게 자체 조건(BASELINE §2 행 + adapter-safety 재검토)만으로 Approved 전환됨 | 보안·비용 |
