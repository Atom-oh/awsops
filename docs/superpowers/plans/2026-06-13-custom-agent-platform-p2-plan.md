...
- [ ] Migration SQL (`-- since: 2.3.0`): `ALTER TABLE integrations ADD COLUMN IF NOT EXISTS direction TEXT NOT NULL DEFAULT 'egress'`, `auth_mode TEXT`, `receive_path TEXT`, `inbound_auth_ref TEXT`, `source_allowlist JSONB NOT NULL DEFAULT '[]'::jsonb`, `trigger_target TEXT`. Add pg_constraint-guarded CHECKs (DO block, like P1): `direction IN ('egress','ingress')`; `kind IN (<egress set>,<ingress set>)` — egress: grafana/datadog/splunk/prometheus/newrelic/notion/confluence/jira/servicenow/slack/github/gitlab/custom_mcp; ingress: cloudwatch_sns/alertmanager/grafana_alert/pagerduty/datadog_monitor/generic_webhook. `CREATE INDEX IF NOT EXISTS idx_integrations_dir_enabled ON integrations (direction, enabled) WHERE enabled = true`. No `schema_migrations` write. **The `kind`/`direction` value sets are the source of truth shared with `web/lib/integration-validation.ts` — comment both.**
...
- [ ] Failing tests: `INTEGRATION_KINDS_EGRESS`/`INTEGRATION_KINDS_INGRESS` (source of truth shared with the migration CHECK — comment); `validateIntegration` rejects bad name (kebab), unknown `kind`, `direction`∉{egress,ingress}, `capability`∉{read,read_write}, egress missing `endpoint`/invalid URL, egress `transport`∉set, ingress missing `auth_mode`; accepts well-formed egress + ingress rows.
...
- [ ] Failing tests: `AgentSpace` gains `enabledIntegrationIds: number[]`; `getAgentSpace` selects `enabled_integration_ids`; `resolveAgent(...)` accepts an optional enabled-integrations arg and, for the custom path, **adds enabled READ integrations' `exposed_tools` to the declared tool set** (then the existing `intersectToolAllowlist` cap applies) and **appends bounded `provided_context` text** to `systemPromptOverride` (cap the injected size — ADR-033). READ_WRITE integrations contribute NO raw tools (writes go via the gate). Built-in path unchanged.
