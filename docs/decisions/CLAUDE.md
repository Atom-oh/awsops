...
| 046 | DevOps RCA 인시던트 오케스트레이터 — EoG 실행 모델 (RCA 결정 **통합**) | **Proposed (2026-06-20)** — 오너 피벗 + 멀티-AI 패널(kimi·glm·agy) 만장일치 PIVOT-TO-RCA-AGENT. 흩어진 RCA 결정(Notion 자체 ADR-001~10·브리프 2개·Stage-3 스펙·supersede된 레지스트리 스펙)을 **단일 ADR로 통합**. **032의 investigation 메커니즘(Lead/Sub ReAct)을 EoG(결정론 컨트롤러+LLM 국소추론)로 승계**(라이프사이클·triage·데이터모델·read-only·flag·022 webhook 유지); **035(k8sgpt)=rules-detect 통합**; **034 writeback=frozen·비범위**(RCA는 `incidents.rca` 로컬); **043 Neptune=S2**(Stage3는 기존 topology 1-hop). EoG⊂AgentCore(Memory·AWS-native), 두 실행경로(챗 ReAct + 인시던트 EoG), read-only(S4 동결). Stage3-first. 구현역할=계획/리뷰 Claude+패널·코드 Codex. 파일 `046-devops-rca-incident-orchestrator-eog.md` |
...
1. 번호: `ls docs/decisions/*.md | tail -1` 로 최신 번호 확인 후 +1 (현재 최고 번호 = **046**)
