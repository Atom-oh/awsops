...
> Scoped to **P3** (consumes the P1f AgentCore fabric + P2 worker backbone + P1e EKS onboarding). Decision support: `~/Documents/K8sGPT_vs_AWSops_Research_20260604/` (deep-research, 22 sources). Depended on ADR-029 for the remediation tier — **ADR-029 was REVERSED 2026-06-11, so that dependency is void**; only the read-only diagnosis scope remains valid (it ships without the remediation tier). / 종전엔 ADR-029(개선 티어)에 의존했으나 **029가 2026-06-11 REVERSED되어 해당 의존성은 무효**; read-only 진단 스코프만 유효(개선 티어 없이 출하).
...
| 4 | ⛔ **ABANDONED (2026-06-11 downgrade)** — routed remediation through ADR-029/036, **now reversed**. No remediation path exists: K8sGPT is read-only diagnosis only; action stays with the operator. *(Original, as history: "Remediation stays in the AWSops worker backbone (ADR-029 + ADR-030/P2), host-scoped, risk-gated, kill-switchable; K8sGPT auto-remediation NOT enabled.")* | Propose/execute separation is moot now that execute (029/036) is abandoned; K8sGPT stays a read-only sensor. |
| 5 | **Privacy + LLM-boundary lock (⚠️ H0-amended — see Post-acceptance)**: the original rule required K8sGPT's `ai.backend = amazonbedrock` in-region. **Post-H0 this is stronger and simpler: K8sGPT runs with NO AI backend at all** (deterministic-only `analyze --output json`), and **all LLM narration happens inside AWSops' AgentCore (Bedrock Haiku 4.5, ap-northeast-2)**. The LLM boundary is therefore **structurally inside AWS/AWSops** — no cluster data is sent to K8sGPT's own LLM path (which couldn't reach in-region Haiku 4.5 anyway). `--anonymize` stays as defense-in-depth (it does **not** mask Event/Describe/ContainerStatus/ConfigMap values/env-var names/image URIs). | Keeping the LLM boundary inside AWSops is the hard control; with K8sGPT deterministic-only there is no external/k8sgpt LLM endpoint to lock down. |
...
| H1 | K8sGPT operator Helm-deployed to one onboarded EKS cluster **deterministic-only (NO `ai.backend`/`--explain` — per the H0 refinement amending Rules 2/5)**, `--anonymize`, read-only RBAC + `--fix` off per Rule 9; `Result` CRDs (`analyze --output json`) + Prometheus metrics flowing. Haiku 4.5 narration is done in **AWSops AgentCore**, not K8sGPT. | `k8sgpt analyze --output json` (no AI backend) returns real findings; metrics scraped; `last_scan_timestamp` exposed. |
...
| ~~H3a~~ | ⛔ **ABANDONED (2026-06-11 downgrade)** — the wiring (K8sGPT → ADR-032 incident → ADR-034 write-back → ADR-029 remediation proposal) depended on the reversed 029/036 + downgraded 032. Not pursued. *(dark code retained, flag-OFF — `web/lib/k8sgpt.ts`'s `raiseIncidentFromFinding` is preserved behind a double gate but is not wired into any route, consistent with ADR-034's retention.)* *(Original, as history: "K8sGPT-sourced incidents → ADR-032 correlation/RCA + ADR-034 write-back; remediation proposals routed to the ADR-029 worker tier, gated, no auto-apply.")* | — (abandoned) |
...
## References / 참고
- **Out-of-band operator install runbook:** `docs/runbooks/k8sgpt-operator-install.md` — Helm deterministic-only (NO `ai.backend`/`--explain`), `--anonymize`, read-only ClusterRole + `--fix` off (Rule 9), pinned version (Rule 7, must match `ADAPTER_K8SGPT_VERSION`), + the `awsops-k8sgpt-result-reader` ClusterRole/Binding to the P1e-mapped `awsops-v2-task` principal. **H1's "done when" is satisfied by this operator runbook + the AWSops-side substrate** (deterministic `Result` CRDs flowing + read-RBAC bound); AWSops executes no install step.
