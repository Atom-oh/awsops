...
§4.4/§4.5는 `scripts/v2/teardown/v1-teardown-4.4-4.5.sh`로 감싸져 있다 — 동일한 절차를 하나의 idempotent 스크립트로 실행한다. **기본값은 DRY-RUN**(해석된 삭제 대상만 출력하고 아무것도 지우지 않음)이며, `CONFIRM=yes <script>`로만 실제 삭제를 수행한다. 이 스크립트의 검증 블록은 async DELETING 상태의 게이트웨이/메모리/인터프리터를 "아직 존재"로 세지 않는다 — §4.4/§4.5 하단의 "빈 결과 기대" 검증 명령도 같은 상태값을 감안해야 한다.

§4.4/§4.5 are wrapped by `scripts/v2/teardown/v1-teardown-4.4-4.5.sh` — one idempotent script running the same steps. **DRY-RUN by default** (prints the resolved deletion plan, deletes nothing); only `CONFIRM=yes <script>` performs actual deletions. Its verification block does not count async-DELETING gateways/memory/interpreter as "still present" — the "expect empty" verification commands later in §4.4/§4.5 should account for the same status value.
