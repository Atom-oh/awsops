...
| 設定 | デフォルト | 説明 |
|------|--------|------|
| **Timeout** | 10 秒 | アップストリームのクエリ実行上限（秒、1–60）。ClickHouse はすべての経路（Explore・サービスグラフ・エージェント）に適用され、コネクタは自身の HTTP タイムアウトをその上に揃えます（有効上限 55 秒 — Lambda の 60 秒制限内に収めるため 56–60 秒の設定は 55 秒に短縮されます）。Prometheus/Mimir は Explore 経路の API `timeout` パラメータとして適用され、コネクタの 12 秒 HTTP タイムアウトの下で 10 秒にキャップされます |
