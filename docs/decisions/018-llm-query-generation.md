...
4. **비용 예산**: 인스턴스당 **ISO 주 3회**, 그리고 **연속 3주** 소진되면 스키마가 바뀔 때까지 정지
   ...
   **연속소진주차수(streak)만** 이어받고 attempts는 이번 주 fresh(0)로 시작한다 — 이번 배포로 이전에 capped
   였던 인스턴스마다 최대 주 1회분(제한적, 상한 있음) 의 재평가 비용이 들지만, 비교 불가능한 해시 알고리즘
   전환에서 스키마를 잘못 귀속시키지 않는 대가로는 합리적이다.
