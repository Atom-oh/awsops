...
- 3 new DB migrations backing the two features above: `sg_rule_activity` (flow sources / rules / rule versions / daily activity / scan runs tables), `network_path_check` (checks / runs / step results tables), and `network_path_runs_error` (adds a nullable `error` column to `network_path_runs` so a failed run — e.g. the "fetch_live_topology is not implemented" case above — has somewhere to record why).
...
- 위 두 기능을 지원하는 신규 DB 마이그레이션 3건: `sg_rule_activity`(flow source/룰/룰 버전/일별 활동/스캔 run 테이블), `network_path_check`(check/run/step 결과 테이블), `network_path_runs_error`(`network_path_runs`에 nullable `error` 컬럼 추가 — 위의 "fetch_live_topology 미구현" 케이스처럼 실패한 run이 그 이유를 남길 곳이 없었음).
