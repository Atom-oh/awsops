...
| GATED operator CI bootstrap | `github_actions_enabled` | Default off; existing-provider lookup and exact origin/production OIDC trust, scoped ECR/ECS permissions, own Aurora metadata and verifier-secret metadata. Migration-secret access is optional and requires an identified reachable private executor; KMS decrypt is limited to explicit keys and approved secrets. No Terraform state, S3, CloudFront or product mutation authority. Bootstrap does not ship a release workflow; UpdateService residual field authority is documented. | ADR-005 operator boundary; [access contract](../reference/github-actions-access.md) |
