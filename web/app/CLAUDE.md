...
## Role
Next.js App Router — 35 pages + 84 API routes (`app/api/`). APIs are thin-BFF: Aurora reads, AWS SDK reads, and AgentCore calls only. Long/OOM-risk work is enqueued via `POST /api/jobs`.
...
- Route handlers declare `export const dynamic = 'force-dynamic'` (consistent across the existing 89 files).
