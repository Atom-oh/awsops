...
- **1.3 alert 경로 외부 발신자 확인**: AWS 네이티브 경로(CloudWatch→SNS→SQS)와 외부 웹훅 경로(Alertmanager/Grafana→앱 HTTP endpoint)를 별도로 확인했다.
  ...
  - 대신 **백필한 실제 `data/config.json`을 직접 열어 확정 증거를 확보**했다: 최상위 키에 `alertDiagnosis`가 **존재하지 않는다**(`isAlertDiagnosisEnabled()`는 `getConfig().alertDiagnosis?.enabled || false`를 반환하므로 이 키 부재 = 항상 `false`). 즉 **POST 핸들러는 파싱·HMAC 검증·로깅 이전에 무조건 503을 반환하는 구조**이며, 이는 로그의 부재가 아니라 실제 운영 설정값으로 확인된 사실이다. `alertSources` 설정도 없어 애초에 라우팅될 소스 자체가 정의돼 있지 않다.
  - → **네 가지 독립 신호(SNS 구독자·CloudWatch Alarm·큐 깊이·config.json의 기능 OFF 확정)가 모두 "위험 없음"으로 일치.** 외부 발신자가 실제로 존재하더라도 v1은 구조적으로 전부 503 거부만 해왔으므로, "성공적으로 수신되던 알림이 폐기 후 조용히 사라지는" 시나리오 자체가 성립하지 않는다. Phase 3(EC2 정지)로 안전하게 진행 가능 — 무음 유실 리스크로 표시했던 유일한 실행조건이 해소됨.
