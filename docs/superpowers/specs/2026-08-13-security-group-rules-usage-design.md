...
(Accepted 2026-08-19, registered as a narrow ADR-005 owner-override exception in the ADR-015 style —
not "no relaxation needed") and reclassifies this feature's Athena/S3 pipeline as sitting inside the
existing read-only invariant for everything except that one exception. Two panel rounds ran across
this document and the ADR: round 1/2 (codex + kiro-cli/claude-fable-5) resolved several MAJORs on IAM
scope and the derived read-only property. Round 3 (PR #230's 5-lens panel) found and this document
resolved further MAJORs — a cost control cited as pre-existing that appeared in no document (now
genuinely introduced in "Flow Log source configuration" §"Cost, result-location, and workgroup
validation preconditions"), an intra-PR contradiction where this document's own §IAM list omitted the
result-prefix read grant ADR-019 ratifies, a missing dedicated-result-prefix/source-disjointness
requirement, an unenforceable `s3:ListBucket` prefix claim (needs an explicit `s3:prefix` condition),
an `athena:StopQueryExecution` ownership-boundary gap IAM cannot close, and an ExternalId-provenance
error (this repo's *existing* read-only cross-account role sometimes has none, per ADR-011 — the
requirement here is a stricter override, not an inheritance). `docs/decisions/BASELINE.md` §2 now
carries the `sg_rule_activity_enabled` register row this document required, labeled
GATED(owner-override 예외) and marked approved-but-unimplemented (no such flag exists in
`terraform/v2/foundation` yet). Owner: 오준석(Junseok Oh), who directed the ADR drafting and each
panel round and approved this Status change after reviewing the findings and fixes.
...
### Cost, result-location, and workgroup validation preconditions (2026-08-19, PR #230 round-3 review)

The daily pipeline runs one Athena query per account/region/source/day with no cost ceiling
specified anywhere — a real gap a stop-time review caught before this section existed. Source
validation additionally requires, and rejects the source otherwise:

- **A per-query byte-scan ceiling.** A new Terraform variable `sg_rule_activity_max_query_bytes`
  (default 100 GiB) bounds the largest single-day scan this feature will run. The configured
  workgroup must have `EnforceWorkGroupConfiguration` enabled and a `BytesScannedCutoffPerQuery` set
  to a value **at or below** `sg_rule_activity_max_query_bytes` — AWSops can only verify this
  (`athena:GetWorkGroup`; no `UpdateWorkGroup` grant exists, see IAM below), never set it, so an
  under-configured or missing cutoff fails validation rather than running unbounded.
- **A dedicated, AWSops-exclusive result prefix — never a bucket root.** The workgroup's query
  result location must resolve to a non-root prefix (e.g. `s3://bucket/awsops-sg-rules/<source-id>/`)
  that this source alone writes to. A result location of `s3://bucket/` would make the "prefix-scoped"
  IAM grant below bucket-wide in practice; validation rejects any location that isn't a distinct,
  non-root prefix.
- **Source-location / result-prefix disjointness.** The configured Flow Log table's data location
  and the workgroup's result prefix must not overlap (neither is a parent/child/equal path of the
  other, even across different buckets in the same account). Without this check, the ratified
  `s3:PutObject` grant on the result prefix could write into the Flow Log source dataset itself.
- **No source-managed encryption AWSops cannot decrypt.** If the Flow Log table's data or the
  workgroup's result location uses SSE-KMS, the source is rejected at validation unless the admin
  also grants `kms:Decrypt` (source) / `kms:Decrypt`+`kms:GenerateDataKey` (result prefix) on that
  specific key to `AWSopsSgRuleAthenaRole` — this feature does not request or assume a KMS grant that
  isn't explicitly configured per source.
...
**2026-08-19 update: resolved (revised 2026-08-19 round 3).** The Athena
`StartQueryExecution`/`StopQueryExecution` calls, the S3 write to the workgroup's results prefix, and
the new cross-account `AWSopsSgRuleAthenaRole` (see IAM section) are not read-only in the literal
sense, and whether they required an ADR-007-tier classification or fell under ADR-005's
AWS-resource-mutation freeze was not this document's decision to make. That classification now has
its dedicated ADR, per BASELINE's ADR-015 precedent (a new ADR + multi-AI panel review, run inside a
`/co-agent:consensus` session, 2026-08-19):
[`docs/decisions/019-athena-flow-log-query-classification.md`](../../decisions/019-athena-flow-log-query-classification.md)
(Accepted). Its ruling, for the two-role split this section and §IAM describe: the reused
`AWSopsReadOnlyRole` for rule inventory needs no exception (it sits inside the existing read-only
invariant, no ADR-007-tier designation). The new, isolated `AWSopsSgRuleAthenaRole` for the
Athena/S3 write path is **not** waved through as "needing no relaxation" — round 3's panel review
flagged that framing as self-contradictory (this role is the repo's first write-capable
cross-account trust relationship, which is exactly what ADR-005 §Decision 3 says may not be
self-argued out of scope). It is instead registered as a narrow, explicit, dated owner-override
exception to ADR-005 (the ADR-015 pattern), valid only while ADR-019 §3b's three controls hold.
`docs/decisions/BASELINE.md` §2 now carries the `sg_rule_activity_enabled` register row, labeled
GATED(owner-override 예외) and citing ADR-019.
...
> `s3:prefix` string-condition (`"Condition": {"StringLike": {"s3:prefix": ["<result-prefix>/*"]}}`)
> on the bucket-ARN statement, or the grant is bucket-wide in practice despite the prose. The
> dedicated-result-prefix and source/result-disjointness requirements in **Flow Log source
> configuration** above are what make "prefix-scoped" actually true for both `GetObject` and
> `ListBucket`.
...
for that source, explicitly excluding `s3:DeleteObject`, any bucket-policy/ACL write action, and any
Glue mutating verb (including partition-mutation verbs like `BatchCreatePartition`/`BatchDeletePartition`
and `UpdateTable` — `INSERT INTO` an existing table or `UNLOAD … TO 's3://…'` need no *Glue* mutating
verb at all, only S3 write, so the result-prefix/source-location disjointness check above is this
...
requires an `ExternalId` condition on the assuming principal plus an explicit principal ARN
restriction to the host account's Athena-worker identity — no wildcard principal, no missing
ExternalId, which would otherwise make this a confused-deputy risk the read-only sibling role never
had. (2026-08-19 correction: this repo's *existing* cross-account convention for the read-only role
is the opposite where it matters — `steampipe.tf`'s 1st-party path has no `ExternalId`, per ADR-011's
relaxation. The mandatory `ExternalId` here is a **stricter override of that relaxation specifically
for this write-capable role**, not an inheritance from it.)
