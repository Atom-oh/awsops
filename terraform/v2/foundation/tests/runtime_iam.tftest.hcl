  ...
  mock_resource "aws_acm_certificate" {
    defaults = {
      ...
      domain_validation_options = [{
        ...
        resource_record_name  = "_fixture.dev.example.com"
        resource_record_type  = "CNAME"
        resource_record_value = "_fixture.acm-validations.aws"
      }]
    }
  }
...
run "defaults_remain_dark" {
  ...
  assert {
    condition = {
      for e in jsondecode(aws_ecs_task_definition.web.container_definitions)[0].environment : e.name => e.value
    }["SSM_RUNTIME_ARN_PARAM"] == ""
    error_message = "A disabled runtime must not advertise an SSM path without a parameter or read grant."
  }
  assert {
    condition = (
...
run "host_core_permissions_and_digest_binding" {
  ...
  assert {
    condition = {
      for e in jsondecode(aws_ecs_task_definition.web.container_definitions)[0].environment : e.name => e.value
    }["SSM_RUNTIME_ARN_PARAM"] == "/ops/awsops-fixture/agentcore/runtime_arn"
    error_message = "Enabled runtime discovery must use the same project path as its SSM permission."
  }
  assert {
