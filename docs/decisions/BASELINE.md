...
| [004](004-agentcore-gateways-runtime.md) | AgentCore 게이트웨이·런타임 | **9 게이트웨이 프로비저닝 / 9 섹션 에이전트 라우트** (external-obs 승격 2026-06-24: Prometheus+ClickHouse) + Memory + Code Interpreter. **§7(2026-07-31 amendment, 사실 기록): Aurora Data API를 쓰는 agent Lambda는 master secret이 아니라 최소권한 롤 `awsops_sql_reader`로 인증 — 명시 allowlist `GRANT SELECT`, 자격증명 보유 컬럼(`eks_registrations.auth`·`accounts.external_id`) 컬럼단위 제외, host 계정 Postgres 전용. 권한 제거이므로 신규 capability 아님** | 운영우수성 |
