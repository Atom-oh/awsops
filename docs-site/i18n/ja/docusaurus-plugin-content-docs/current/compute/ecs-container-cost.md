...
サービスごとの CPU コストと Memory コストを比較します。v2 では積み上げバーの代わりに**共通スケールのグループバー**（2 つの $ シリーズが 1 つのスケールを共有 — 実際の比率を保持）でレンダリングされ、クラスター/サービスラベル・FARGATE 限定・上位 10・500 行超過時の「標本基準」表記が適用されます。
