...
2026-09-01부터 Terraform 게이트 위에 **런타임 일시중지 토글**이 추가되었다: Aurora `app_settings`의 `diagnosis_notify_paused` 키(관리자 전용 `PUT /api/diagnosis/notify`, 배포 불필요). 시맨틱은 토픽 미구성과 동일 — digest worker는 발행을 생략하되 `notified_at`을 그대로 스탬프한다(중지 중 완료된 리포트는 이메일에서 **탈락**하며, 재개 시 소급 발송되지 않는다). 플래그 **조회 실패는 발송 쪽으로 fail-open** — 설정 조회 장애가 알림을 조용히 죽여서는 안 된다(로그 남김). 관리자 테스트 발송(`subscribers/test`)은 일시중지의 영향을 받지 않는다(의도된 carve-out — 중지 상태에서도 배달 경로 검증이 가능해야 한다). digest 주기(15분)보다 짧은 일시중지는 아무것도 탈락시키지 않을 수 있다(플래그는 실행 시점에 샘플링). 일시중지는 편의 제어다 — 오발송 차단이 목적이면 하드 스톱은 구독 해지 또는 `diagnosis_notify_enabled=false` 배포다. 배달 기록은 로그가 아니라 **Aurora 내구 레코드**다: `diagnosis_reports.notify_outcome`(CHECK 제약 어휘 `emailed`[SNS publish가 MessageId를 반환 — 발행 수락 증거이지 최종 이메일 배달 증거는 아니다] · `emailed_failopen`[플래그 조회 실패 fail-open 중 발행] · `publish_failed`[MessageId 없음 — 그래도 backlog는 배수(drain)되며 **재시도하지 않는다**] · `dropped_paused` · `skipped_no_topic`). agent 읽기 뷰 `sql_reader.diagnosis_reports`는 같은 마이그레이션에서 이 컬럼을 포함해 재투영된다.
...
Since 2026-09-01 a **runtime pause toggle** sits above the Terraform gate: the `diagnosis_notify_paused` key in Aurora `app_settings` (admin-only `PUT /api/diagnosis/notify`, no deploy needed). Semantics match a missing topic — the digest worker skips publishing but still stamps `notified_at` (reports completed while paused are **dropped** from email, never re-blasted on resume). A flag-READ failure **fails open to publishing** (a broken settings read must not silently kill notifications; it is logged). The admin test send (`subscribers/test`) is deliberately NOT covered by the pause — delivery-path verification must stay possible while paused. A pause shorter than the digest cadence (15 min) may drop nothing — the flag is sampled per run. The pause is a convenience control; the hard stop for a mis-addressed blast is unsubscribing or deploying `diagnosis_notify_enabled=false`. The delivery record is a DURABLE Aurora column, not logs: `diagnosis_reports.notify_outcome` (CHECK-constrained vocabulary — `emailed` [SNS publish returned a MessageId: publish ACCEPTANCE, not end-to-end email delivery], `emailed_failopen` [published under a failed-open flag read], `publish_failed` [no MessageId — the backlog still drains and is **never retried**], `dropped_paused`, `skipped_no_topic`). The agent read view `sql_reader.diagnosis_reports` is re-projected with the column in the same migration.

2026-09-02부터 같은 토픽에 **세 번째 메시지 클래스 — 컴플라이언스 벤치마크 완료 통지** — 가 추가되었다(gap-audit L192, v1 notifyBenchmarkCompleted 패리티). 발행 주체는 여전히 **worker 전용**(`compliance.notify_completed`, `diagnosis/notify._client` 재사용)이며 신규 IAM/Terraform 변경은 없다(동일 `DIAGNOSIS_SNS_TOPIC_ARN` env + 기존 단일-토픽 `sns:Publish`). 벤치마크 실행이 **성공적으로** 완료·persist된 뒤에만 벤치마크명·scope·전체/통과/실패 건수·통과율·`/compliance` 링크를 발송한다. 홍수 방지: 실행은 사용자 트리거 가능하므로(비관리자 `POST /api/compliance/run`) **벤치마크당 60분 dedup 창**이 재발송을 차단한다 — 창 선점은 발행 **전에** 원자적으로 수행된다(벤치마크별 advisory lock으로 직렬화된 단일 `UPDATE … NOT EXISTS … RETURNING`으로 `compliance_runs.notified_at`을 선점; 동시 실행 N건이 각자 검사를 통과해 전부 발송하는 check-then-publish 레이스 불가). publish 실패 시에도 선점된 창은 유지된다(재시도 발송 없음). 선점 대상 행 자체도 `notified_at IS NULL`이어야 하므로, 이미 통지된 실행의 수동 SFN 재구동은 자기 창을 재선점해 재발송할 수 없다(이때 skip 계열 outcome은 기존 emailed/publish_failed 기록을 덮어쓰지 않는다). 다이제스트가 대체했던 리포트당 즉시 메일 패턴을 무제한으로 재도입하지 않기 위한 장치다. 메시지에는 `awsops_class=compliance_completed` MessageAttribute가 실려 구독자가 SNS 필터 정책으로 진단 메일과 분리 수신할 수 있다. `diagnosis_notify_paused` 일시중지는 이 메일에도 동일하게 적용된다(조회 실패 fail-open). 배달 기록은 `compliance_runs.notify_outcome`(diagnosis와 동일 어휘 + `skipped_dedup`)으로 내구 저장되며, `sql_reader.compliance_runs` 뷰는 같은 마이그레이션에서 재투영된다. Subject는 ASCII 고정(비-ASCII Subject는 SNS가 거부).

Since 2026-09-02 the same topic carries a **third message class — compliance benchmark completion notices** (gap-audit L192, v1 notifyBenchmarkCompleted parity). Publishing remains **worker-only** (`compliance.notify_completed`, reusing `diagnosis/notify._client`) with zero new IAM/Terraform (same `DIAGNOSIS_SNS_TOPIC_ARN` env + the existing single-topic `sns:Publish`). Sent only after a run **successfully** completes and persists (benchmark, scope, total/passed/failed counts, pass rate, `/compliance` link). Flood guard: runs are user-triggerable (non-admin `POST /api/compliance/run`), so a **60-minute per-benchmark dedup window** blocks re-blasts — the window is claimed atomically BEFORE the publish (a single `UPDATE … NOT EXISTS … RETURNING` on `compliance_runs.notified_at`, serialized by a per-benchmark advisory lock, so N concurrent runs cannot each pass a check and all publish), and a publish failure keeps the claim (no retry-blast); the target row itself must have `notified_at IS NULL`, so a manual SFN re-drive of an already-notified run cannot re-claim its own window (skip-class outcomes never overwrite an existing emailed/publish_failed record) — the per-report instant-mail pattern the digest retired is not reintroduced unbounded. Messages carry an `awsops_class=compliance_completed` MessageAttribute so subscribers can filter the class via SNS filter policies. The `diagnosis_notify_paused` pause governs this mail too (read failure fails open). Delivery is durably recorded in `compliance_runs.notify_outcome` (the diagnosis vocabulary + `skipped_dedup`); `sql_reader.compliance_runs` is re-projected in the same migration. The Subject is pinned ASCII (SNS rejects a non-ASCII Subject).
...
- `terraform/v2/foundation/migrations/*_app_settings.sql` · `*_diagnosis_reports_notify_outcome.sql` — 설정 저장소 + 내구 배달 레코드(CHECK 어휘, sql_reader 뷰 재투영)
- `scripts/v2/workers/diagnosis/db.py` `mark_notified(..., outcome=)` — notified_at 스탬프와 동일 UPDATE로 outcome 기록 (pre-migration fallback 포함)
