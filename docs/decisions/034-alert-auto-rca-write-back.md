...
|---|---|---|
| **extends** | ADR-032 | The autonomous lifecycle produces the RCA; ADR-034 is its bidirectional **output/enrichment** channel (write-back to the source incident record). ADR-032 (Proposed) **will supersede** ADR-009 once Accepted — until then ADR-009 remains in force and ADR-034 builds on the ADR-009 diagnosis. |
...
| **extends** | ADR-029 | Write-back to OpsCenter/Incident Manager/Grafana/ITSM is a **mutating action on the observability plane** → governed by ADR-029, as a **lower-risk "observability-metadata write" tier** distinct from infrastructure mutation. Annotate/create/update is the default; **silences are higher-risk and separately gated**. ⚠️ **Account-boundary caveat**: ADR-029 v1 restricts *all* mutations to the **host account only** and the executor Lambdas reject non-host `accountId`. Writing incidents **in the customer (target) account** (see relation to ADR-018/008 below) therefore requires ADR-029 to first lift the host-only restriction *for this observability-write tier* — this is the "future ADR" ADR-029 anticipates. **Until that lift is Accepted, v1 write-back is limited to the host account**; cross-account write-back is gated behind it. |
