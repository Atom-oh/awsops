...
- **AI:** Bedrock **Sonnet 5 / Opus 4.8 / Haiku 4.5** + AgentCore (Strands, reuses `agent/agent.py`; routes via `GATEWAYS_JSON`). Live AWS queries go through **AgentCore MCP Lambda tools** (`agent/lambda/*.py`), never inline in the BFF. Config source-of-truth = **SSM** `/ops/awsops-v2/agentcore/{runtime_arn,interpreter_id,memory_id}` (read at runtime; no ECS `valueFrom` to avoid a race).
