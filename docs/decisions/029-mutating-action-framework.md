...
> **⛔ REVERSED (AWS-resource scope) / PARTIAL-UNFREEZE (external DATA, via ADR-040)** — owner decision via 3-AI consensus (kiro/codex/gemini; see `docs/reviews/2026-06-11-high-risk-adr-reversal-consensus.md`). The **AWS-resource** mutating-action direction is **abandoned**: AWSops stays a read-only ops dashboard for AWS resources; infra mutation is left to the operator's SSM/Change Manager/IaC/console. The AWS-resource substrate (with ADR-036) was built but shipped flag-OFF and is now **frozen** — `remediation_enabled` stays `false` permanently (**do-not-enable**); the harmless dark code is kept, not deleted.
>
> **Scope clarification (2026-06-16):** "frozen" applies to the **AWS-resource** scope. Per **ADR-040/041**, the six **controls** (action_catalog facade, 4-eyes, kill-switch, audit) + the per-action **lambda executor** may be **reused for non-AWS-resource external DATA writes** (Slack/Notion/Jira). "Decoupled" there means a **separate gating layer** (own `integrations_write_enabled` flag + dedicated kill-switch + no-AWS-mutation IAM role) running on the **shared P2 worker Step Functions spine** (ADR-036 single engine, **not** a second state machine) — not a revival of the AWS-resource SSM/Change-Manager automation. The "Accepted" record below is retained as history. / "동결"은 **AWS-리소스 스코프**에 한정; ADR-040/041에 따라 통제·lambda executor는 비-AWS 외부 DATA write에 재사용 가능("decoupled"=별도 게이팅 층[전용 플래그/킬스위치/IAM], 단 공유 P2 SFN spine 사용·별도 엔진 아님).
...
A single SSM Parameter `/ops/awsops-v2/mutating-actions/enabled` (default `false`) gates every executor Lambda. Setting it to `false` causes in-flight Step Functions executions to fail-fast at the next state transition and prevents new plans from executing (planning still works, so dry-run remains available). The dashboard surfaces this state and the email of whoever last toggled it.

단일 SSM Parameter `/ops/awsops-v2/mutating-actions/enabled` (기본값 `false`)가 모든 executor Lambda를 게이트한다. `false`로 변경하면 진행 중인 Step Functions 실행은 다음 상태 전이에서 즉시 실패하고, 새 플랜은 실행되지 않는다(플래닝은 여전히 작동하므로 dry-run은 가용). 대시보드는 이 상태와 마지막 토글한 사용자의 이메일을 노출한다.
...
## Consensus Revision (2026-06-09) / 합의 리뷰 개정

Multi-AI consensus review (codex·gemini = REVISE, kiro = ACCEPT-WITH-CHANGES; Claude chair). The six controls are **unanimously sound and substrate-agnostic**; the body's mechanism/IaC wording predates ADR-030 and the now-Accepted ADR-036 and is **revised here**. The items below **supersede** the corresponding sections above. (멀티AI 합의 리뷰 — 6대 통제는 건전, 아래가 본문의 해당 부분을 승계한다.)

1. **Execution mechanism → ADR-036 (Accepted).** ADR-029 is a **substrate-agnostic controls spec**, not an execution design. The body's "Option 2: per-action Lambda + dedicated Step Functions / ~10 new Lambdas / one SFN per action" is superseded by ADR-036's hybrid: P2 worker backbone (`/api/jobs`·`worker_jobs`) as control-plane, SSM Automation/Change Manager for AWS-resource actions, P2 Lambda/Fargate for K8s/app-state. The six controls apply to whichever executor 036 routes to. (codex CRITICAL; gemini/kiro MAJOR) / 실행 메커니즘은 036 하이브리드로 위임; 029는 통제 사양.
2. **IaC + state → v2.** "CDK stack"/"MutatingActionsStack" → **Terraform** (`terraform/v2/foundation/`); "EC2 role" → ECS/Fargate task roles + per-action SSM `AutomationAssumeRole`/Lambda roles; "`data/config.json`" → **Aurora**. Implementation-plan steps are substrate-neutral, deferring to 036. (codex/gemini/kiro MAJOR) / IaC·상태를 v2(Terraform/Fargate/Aurora)로 현행화.
3. **Kill-switch path** corrected to the v2 SSM convention `/ops/awsops-v2/mutating-actions/enabled` (a leading `aws…` component is reserved) — fixed inline above. (codex MAJOR) / 킬스위치 경로 v2 규약으로 정정(인라인 반영).
4. **Audit/approval correctness.** (a) Operator identity is **not** "forwarded into CloudTrail `sourceIPAddress`/`userAgent` via Lambda logs" — set a **custom SDK User-Agent at call time** and persist the **authenticated principal in the P2/Aurora audit record**; CloudTrail is **defense-in-depth, not a synchronous gate** (eventual delivery). (b) "Fail if any of three sinks fails" → **pre-mutation intent persistence + terminal reconciliation**: only the synchronously-controlled sinks (S3 Object-Lock + Aurora) fail-fast; a post-mutation audit-write failure enters a **degraded-audit** state, never silent. (codex MAJOR, kiro MINOR) / 감사·승인 정정: CloudTrail은 게이트 아님, 인증 주체는 Aurora 감사에 기록, 3-싱크 동기 게이트 가정 제거.
5. **IAM condition realism.** The uniform `aws:ResourceTag/awsops:managed-by-event` condition is **unsupported by many `Modify*`/service APIs**; each catalog action needs service-specific IAM validation and **exact ARN/ID allowlists** where tag/resource-level conditions are unavailable. (codex MAJOR) / 일부 API는 태그 조건 미지원 → 작업별 IAM 검증 + 정확 ARN allowlist.
6. **Dry-run + rollback.** "Same-shape parallel dry-run for every phase" is not universal (TOCTOU, APIs without check-mode) → use 036's per-action **dry-run contract** + preconditions/state-locks. A **failed rollback** → terminal **`MANUAL_INTERVENTION_REQUIRED`**, not infinite retry. (codex MAJOR, gemini MINOR) / dry-run은 036 계약 위임, 롤백 실패는 수동개입 종료상태.
7. **Cross-account = toggle, not limit.** Host-account-only is a safety toggle (`ALLOW_CROSS_ACCOUNT_MUTATION=false`); 036/SSM `TargetLocations`+`AutomationAssumeRole` (ADR-008) enable member-account execution when lifted. (gemini MINOR, codex MAJOR) / 교차계정은 토글, 아키텍처 제약 아님.

**relates** ADR-036 (execution substrate), ADR-030 (Aurora/Fargate). / 관계: 036(실행 substrate)·030(Aurora/Fargate).
