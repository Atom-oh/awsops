...
- Explore NL→PromQL generation is anchored to the datasource's schema vocabulary: every metric-name token in the generated query must exist in the schema the model was shown (strings, label matchers, grouping-clause labels, and durations excluded) — an unknown name (e.g. a recording rule absent from the target, like ':node_memory_MemAvailable_bytes:sum') triggers one corrective retry and then an honest error naming the tokens, instead of returning a query that parses but can never match; the prompt additionally steers PromQL away from recording-rule names and label-mismatched vector arithmetic.
...
- Explore 자연어→PromQL 생성이 데이터소스 스키마 어휘에 앵커링됨: 생성 쿼리의 모든 메트릭 이름 토큰이 모델에게 보여준 스키마에 존재해야 함(문자열·라벨 매처·그룹핑 절 라벨·기간 제외) — 없는 이름(예: 대상에 없는 recording rule ':node_memory_MemAvailable_bytes:sum')은 1회 교정 재시도 후에도 남으면 해당 토큰을 명시한 오류로 반환(파싱은 되지만 절대 매칭될 수 없는 쿼리를 건네지 않음); 프롬프트도 recording-rule 이름과 라벨 불일치 벡터 연산을 피하도록 보강.
