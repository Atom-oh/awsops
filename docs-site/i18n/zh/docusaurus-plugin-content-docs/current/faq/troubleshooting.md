...
AI 诊断不是由 Web 直接执行的，而是由**异步 Worker 层**在后台生成的只读报告（base 8 个分区 / deep 15 个分区）。因此"没有响应"并不等于"失败"。
...
3. **deep + Opus 模型** — 在 deep 诊断（15 个分区）中选择 Opus 模型时，会应用成本门禁且耗时更长。想要快速查看，请使用默认 Sonnet 的 base 诊断。
...
AI 助手使用只读工具（约 120 个）查询实时 AWS 数据，会话保存在 Aurora 中。
