...
- Chart quick wins: the Compliance page gains v1's Alarms by Section bar chart (alarm counts per section from the same client rollup the pass-rate list uses; zero-alarm sections get no bar, an all-clear run omits the chart), the S3 page gains a Security Status flag-bar chart (bucket counts per Private/Public/Versioned/Logging flag via a new generic independent-flag spec option — an unknown (access-denied) bucket counts into neither Private nor Public, and the Private/Public bars populate once the newly synced bucket-policy public flag lands on bucket rows after the next sync), and the Subnets page gains a Subnets-per-VPC count bar.
...
- 차트 퀵윈: Compliance 페이지에 v1의 Alarms by Section 막대 차트(pass-rate 목록과 동일한 클라이언트 롤업 기반 섹션별 Alarm 건수 — 0건 섹션은 막대 없음, 전부 통과 run은 차트 생략), S3 페이지에 Security Status 플래그 바 차트(Private/Public/Versioned/Logging 플래그별 버킷 수 — 신규 generic 독립 플래그 spec 옵션; 미상(권한 거부) 버킷은 Private/Public 어느 쪽에도 세지 않으며, Private/Public 막대는 버킷 정책 공개 플래그가 새로 sync에 추가되어 다음 sync 후 채워짐), Subnets 페이지에 VPC별 서브넷 수 카운트 바 추가.
