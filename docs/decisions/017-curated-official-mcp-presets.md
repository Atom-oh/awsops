...
게이트 2개, 둘 다 기본 false:
- **`official_mcp_enabled`** (`ai.tf`) — 벤더 **호스팅** 공식 MCP 3종(Datadog·Dynatrace·New Relic)을 external-obs `mcpServer` target으로 등록. 종전 `do-not-enable` 이었던 활성화 차단 사유 2개는 이번 개정으로 해소되었다: ① 런타임 per-preset 툴 allowlist가 구현됨(아래 §Decision — fail-closed), ② 자체 호스팅 프리셋이 이 경로에서 **제거**되어 도달성 미검증 문제 자체가 소멸(남은 3종은 공용 SaaS 엔드포인트). 이제 일반 GATED — 켜려면 선행조건 `agentcore_enabled`+`integrations_enabled`(둘 다 없으면 silent SKIP)에 더해 프리셋별 `official_mcp_read_only_ack`(fail-closed)까지 필요하다. (남는 잔여: managed-egress 경로의 connect-time SSRF/DNS-rebinding 검증 불가는 activation blocker가 아니었고 계속 standing이다 — 목적지 리스크는 코드 강제 `allowed_host_suffixes` 핀으로 한정. ADR-004/007 follow-up 참조.)
