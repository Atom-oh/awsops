...
## ADR
`docs/decisions/ADR-*.md` (001–036). v2 관련: **030**(ECS Fargate + Aurora, Accepted) 채택; **029**(변경 작업 프레임워크 — 메커니즘은 036이 정제, 6대 통제 유지), **031**(런타임 커스터마이즈 에이전트), **032**(자율 인시던트 라이프사이클), **033**(AIOps LLM 비용 최적화), **034**(알림 자동 RCA 라이트백), **035**(K8sGPT 하이브리드 인클러스터 진단), **036**(변경·조치 실행 substrate — SSM Automation+Change Manager × P2 하이브리드)은 Proposed. 신규 ADR 번호 = 최고번호+1. ADR 인덱스/정정 노트는 `docs/decisions/CLAUDE.md`.
