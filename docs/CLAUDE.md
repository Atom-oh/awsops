...
| [decisions/](decisions/) | **결정 단일 진실 = `BASELINE.md`** + 통합 ADR 001~020 + `ADR-MAPPING.md` (옛 ADR 001~046 본문은 git tag `adr-legacy-2026-06-22`) |
...
- 모든 신규 문서는 **한국어/영어 병기** — 예외: (a) Claude Code가 세션마다 자동으로 로드하는 컨텍스트
  ...
  병기 규칙 그대로 유지.
- 결정 현행 진실 = `docs/decisions/BASELINE.md`. 새 ADR = 통합 ADR 최고번호 + 1 (현재 **020**), 같은 PR에서 BASELINE 갱신
