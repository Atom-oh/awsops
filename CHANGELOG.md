...
### Fixed
- Explore NL→PromQL generation is anchored to the datasource's FULL cached metric list: every metric-name token in the generated query must exist there (comments, strings, label matchers, grouping-clause labels, and number/duration literals excluded; the gate is skipped when the vocabulary is absent or connector-truncated) — an unknown name (e.g. a recording rule absent from the target, like ':node_memory_MemAvailable_bytes:sum') triggers one corrective retry that shows the model its previous answer, then an honest error naming the tokens instead of a query that parses but can never match; the prompt additionally steers PromQL away from recording-rule names and label-mismatched vector arithmetic. Recorded as ADR-018's third (live, draft-only) generation path.
...
### Fixed
- Explore 자연어→PromQL 생성이 데이터소스의 전체 캐시 메트릭 목록에 앵커링됨: 생성 쿼리의 모든 메트릭 이름 토큰이 그 목록에 존재해야 함(주석·문자열·라벨 매처·그룹핑 절 라벨·숫자/기간 리터럴 제외; 어휘가 없거나 커넥터 절단이면 게이트 생략) — 없는 이름(예: 대상에 없는 recording rule ':node_memory_MemAvailable_bytes:sum')은 직전 답을 보여주는 1회 교정 재시도 후에도 남으면 해당 토큰을 명시한 오류로 반환(파싱은 되지만 절대 매칭될 수 없는 쿼리를 건네지 않음); 프롬프트도 recording-rule 이름과 라벨 불일치 벡터 연산을 피하도록 보강. ADR-018의 제3(라이브·초안 전용) 생성 경로로 기록.
