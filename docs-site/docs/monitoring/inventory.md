...
:::tip 스냅샷 기반 데이터
스냅샷은 성공한(부분 실패가 아닌) 인벤토리 sync 실행마다 계정별로 Aurora(`inventory_snapshots`)에 기록됩니다(부분 실패 run은 기존 마지막 값을 보존하므로 일자 포인트가 비어 있을 수 있음) — 대시보드 로드와는 무관하며, 조회 시 추가 AWS API 호출이 없습니다.
:::
