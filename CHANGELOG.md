...
- Inventory/datasources i18n completion: the generic inventory pages (CloudFront/DynamoDB/WAF and every other type) and the datasources hub (form, tab, card dashboard, Explore) now translate every Korean UI string into en/zh/ja — 17 remaining unregistered strings and the donut-title pattern ('<label> 분포 (표본 기준)') were registered, and a lockstep test pins the coverage so a new untranslated string on those surfaces fails CI. Column/spec labels deliberately stay English (technical identifiers keep one form across locales).
...
- 인벤토리/데이터소스 i18n 완결: 범용 인벤토리 페이지(CloudFront/DynamoDB/WAF 포함 전 타입)와 데이터소스 허브(폼·탭·카드 대시보드·Explore)의 모든 한국어 UI 문자열이 en/zh/ja로 번역됨 — 잔여 미등록 17건과 도넛 제목 패턴('<라벨> 분포 (표본 기준)')을 등록하고, 락스텝 테스트가 커버리지를 고정(해당 표면의 신규 미번역 문자열은 CI 실패). 컬럼/스펙 라벨은 의도적으로 영어 유지(기술 식별자는 로케일 간 단일 표기).
