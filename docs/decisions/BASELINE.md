...
| **GATED** | 큐레이션 공식 MCP 프리셋 (external-obs `mcpServer` target) | `official_mcp_enabled` | **벤더 호스팅 3종만**(Datadog·Dynatrace·New Relic — 2026-08-05 개정으로 자체 호스팅 clickhouse/tempo/jaeger/grafana/splunk 프리셋 제거). 엔드포인트는 `official_mcp_endpoints` map(데이터, https만, `allowed_host_suffixes` 핀), capability=read. 강제 2중: ① 프리셋별 `official_mcp_read_only_ack[preset_key]`(= 검토한 엔드포인트 URL, 미설정/불일치 ⇒ SKIP+retire), ② **런타임 fail-closed 툴 allowlist** — catalog `tool_allowlist`(벤더 문서 전사, 추측 금지) → `OFFICIAL_MCP_TOOL_ALLOWLIST_JSON` 런타임 env → agent.py가 `*-mcp-server-target___*` 툴을 교집합(공란 ⇒ 툴 0개, env 부재 ⇒ 전부 드랍). 종전 do-not-enable 사유 2건(allowlist 부재·자체호스팅 도달성)은 해소 — 일반 GATED. Prometheus/Mimir(공식 MCP 無)·Tempo/Jaeger(인-바이너리)·Notion(hosted 3LO 전용)은 자체 람다/기존 방식 유지, Grafana/Splunk는 미지원. `custom_mcp`(임의 엔드포인트) 등록은 그대로 폐기 | ADR-017 |
| **GATED** | ClickHouse 공식 MCP stdio 내장 (AgentCore 런타임 내 `mcp-clickhouse` 서브프로세스) | `CLICKHOUSE_OFFICIAL_MCP` (런타임 env, `make agentcore` 시 기록) | 기존 Datasources 등록(integrations secret `clickhouse` kind-mirror)의 endpoint/자격증명 재사용 — 신규 토큰·서버·terraform 없음. read-only 코드 고정(`CLICKHOUSE_ALLOW_WRITE_ACCESS=false`·`ALLOW_DROP=false`). ON이면 자체 람다 clickhouse 툴은 런타임에서 드랍(상호배제) | ADR-017 |
...
| [017](017-curated-official-mcp-presets.md) | 큐레이션 공식 MCP 프리셋 | (개정 2026-08-05) 벤더 호스팅 3종(Datadog·Dynatrace·New Relic)만 external-obs `mcpServer` target + 런타임 fail-closed 툴 allowlist; ClickHouse는 공식 `mcp-clickhouse`를 런타임 stdio 내장(`CLICKHOUSE_OFFICIAL_MCP`, Datasources 자격증명 재사용); Tempo/Jaeger 자체 람다 유지, Grafana/Splunk 미지원, `custom_mcp`(임의 BYO) 폐기 불변 | 운영우수성·보안 |
