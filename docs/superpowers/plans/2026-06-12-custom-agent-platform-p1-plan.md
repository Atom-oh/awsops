...
- [ ] Write the migration SQL: first line `-- since: 2.2.0`. (a) `CREATE TABLE IF NOT EXISTS` for skills/agents/agent_skills/agent_spaces/customization_audit using the **verbatim baseline DDL** (drift fix — no-op where baseline already created them). (b) `ALTER TABLE … ADD COLUMN IF NOT EXISTS`: `agents.agent_type TEXT NOT NULL DEFAULT 'generic'` + `ADD CONSTRAINT … CHECK (agent_type IN ('generic','on_demand','triage','rca','mitigation','evaluation'))` (guard with a `DO $$ … IF NOT EXISTS` block so re-run is safe), `agents.gateways JSONB NOT NULL DEFAULT '[]'::jsonb`, `agents.response_language TEXT`, `skills.agent_types JSONB NOT NULL DEFAULT '["generic"]'::jsonb`, `skills.reference_keys JSONB NOT NULL DEFAULT '[]'::jsonb`, `agent_spaces.enabled_integration_ids JSONB NOT NULL DEFAULT '[]'::jsonb`, `agent_spaces.response_language TEXT`. (c) `CREATE TABLE IF NOT EXISTS integrations` (id BIGSERIAL PK, name TEXT UNIQUE NOT NULL, kind TEXT NOT NULL, description TEXT NOT NULL DEFAULT '', endpoint TEXT, transport TEXT CHECK (transport IN ('sigv4','oauth_client_credentials','oauth_3lo','api_key')), credentials_ref TEXT, private_connection_ref TEXT, capability TEXT NOT NULL DEFAULT 'read' CHECK (capability IN ('read','read_write')), exposed_tools JSONB NOT NULL DEFAULT '[]'::jsonb, provided_context JSONB NOT NULL DEFAULT '{}'::jsonb, write_action_refs JSONB NOT NULL DEFAULT '[]'::jsonb, tier TEXT NOT NULL DEFAULT 'custom' CHECK (tier IN ('builtin','custom')), enabled BOOLEAN NOT NULL DEFAULT false, created_by TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()) — created but **unused by the runtime in P1**. (d) `UPDATE agents SET gateways = jsonb_build_array(gateway) WHERE gateways = '[]'::jsonb AND gateway IS NOT NULL`. (e) Seed `devops`/`security`/`finops` via `INSERT … (name,description,persona,routing_keywords,gateway,gateways,agent_type,tier,enabled) … ON CONFLICT (name) DO NOTHING` (devops→primary `ops`, gateways `["ops","monitoring","container","iac","network"]`; security→`security`; finops→`cost`). **Do NOT write `schema_migrations`** (the runner stamps it).
- [ ] `node scripts/v2/migrations-p1.itest.mjs` green; `DRY_RUN=1 OFFLINE=1 make migrate` lists the new file without error. Commit `feat(agent-platform): P1 idempotent migration — drift fix + frontier seeds + integrations table`.
...
- [ ] Failing `unittest` (stdlib, no external deps — uses fake tool objects exposing `.tool_name`): `_filter_tools(tools, allowlist)` returns only tools whose `tool_name ∈ allowlist`, preserving order; `allowlist` empty list or `None` ⇒ returns all tools unchanged; names in allowlist but absent from tools are ignored; duplicate-safe.
- [ ] Implement pure `_filter_tools(tools, allowlist)` in `agent.py`. In `handler()`, read `allowlist = payload.get('toolAllowlist')`; immediately after `tools = get_all_tools(mcp_client)`, set `tools = _filter_tools(tools, allowlist)` so the filtered set flows into BOTH the `## Available Tools (N)` prompt section AND `Agent(model, tools=tools, …)`. Empty/absent allowlist ⇒ current behavior (all tools). Keep the MCP-failure fallback path unchanged.
- [ ] `python3 -m unittest agent.test_agent` (run from repo root) green. Commit `fix(agent-platform): enforce resolved toolAllowlist in agent.py (close ADR-031 no-op)`.
...
- [ ] Failing tests: `KNOWN_TOOL_CATALOG.security` is a non-null array containing the IAM MCP tool names (the deployed slice; enumerate from `agent/lambda/aws_iam_mcp.py` tool registrations); `intersectToolAllowlist('security', [<declared incl. an unknown tool>], null)` drops the unknown (now that the catalog is known); a still-`null` gateway (e.g. `iac`) is degrade-safe (no drop); a non-empty `space.toolAllowlist` still only REMOVES.
- [ ] Populate `KNOWN_TOOL_CATALOG` entries for the gateways whose tool sets are confidently enumerable from `agent/lambda/*` (at minimum `security` from `aws_iam_mcp.py`, `network` from `network_mcp.py`/`flowmonitor.py`/`reachability.py`); leave the remainder `null` with a comment that `null` = degrade-safe "inventory unknown here" and that tightening only ever ADDS restriction.
- [ ] `cd web && npx vitest run lib/agent-space.test.ts` green. Commit `feat(agent-platform): fill KNOWN_TOOL_CATALOG for deployed gateway slices`.
...
- [ ] Failing test (match the existing `catalog.test.ts` getPool-mock pattern): `isCustomAgentEnabled(name)` issues a direct indexed query `SELECT 1 FROM agents WHERE name=$1 AND tier='custom' AND enabled=true` and returns `true` only for an enabled custom row; `false` for disabled / missing / builtin / on query error (fail-closed).
- [ ] Implement `isCustomAgentEnabled(name): Promise<boolean>` in `catalog.ts` (no cache — authoritative). In `web/app/api/chat/route.ts`, after `pickCustomAgent` selects a custom `routeKey`, `await isCustomAgentEnabled(routeKey)`; if false, **do not invoke the custom agent** — fall through to the built-in gateway. This makes a disable take effect immediately (bypassing the 30 s `catalog-source` cache, which stays only for *enable* latency).
...
- [ ] Failing validation tests: add `AGENT_TYPES = ['generic','on_demand','triage','rca','mitigation','evaluation']`; `validateAgent` rejects an `agentType` not in the set (default `'generic'` accepted), rejects `gateways` not all in `KNOWN_GATEWAYS` (empty allowed), keeps existing rules; `validateSkill` accepts `agentTypes` (string[] ⊆ AGENT_TYPES, default `['generic']`) and `referenceKeys` (string[]).
- [ ] Failing catalog tests (getPool-mock pattern): `upsertAgent` persists `agent_type`, `gateways`, `response_language`; `upsertSkill` persists `agent_types`, `reference_keys`; `listAgentsWithSkills` returns the new agent fields and `listSkills` returns skill `agent_types`. Extend `AgentInput`/`SkillInput`/`AgentWithSkills` types accordingly (keep `gateway` for back-compat = `gateways[0]`).
- [ ] Implement validators + catalog SQL columns + extend `POST /api/customization` to read/forward the new fields (admin gate + audit unchanged).
...
- Task 2: `python3 -m unittest agent.test_agent`.
