...
- Chart form-fit pass (dataviz methodology): the home 'Monthly Cost Impact (est.)' list becomes a DIVERGING bar chart (signed bars around a shared zero axis — increase on the warm/negative pole, decrease on the positive pole, symmetric scaling so equal magnitudes read as equal lengths; the 30d count delta stays as a muted sub-figure, ordering and every honesty gate unchanged), and two close-magnitude comparisons trade donuts for ranked horizontal bars — the home 'EC2 instance types' breakdown (nominal categories with many close values — exact ranking beats slice-angle comparison, no slice ceiling) and Bedrock's 'Cost per model' (now reads as one comparable block with its sibling invocation bars). Genuinely part-to-whole donuts (categories, pod status, service types, severity, job status) are deliberately kept. Colorblind safety of the diverging pair was validated (CVD ΔE 13.9 light / 12.2 dark, both ≥ the 8 target); every bar carries a visible signed value label.
...
- 차트 형태 적합화(dataviz 방법론): 홈 '월 비용 영향 추정' 리스트를 DIVERGING 바 차트로 교체(공유 0축 기준 서명 바 — 증가는 warm/negative 극, 감소는 positive 극, 대칭 스케일로 같은 크기는 같은 길이; 30일 수량 델타는 보조 수치로 유지, 정렬과 모든 정직성 게이트 불변). 근접 크기 비교 2곳은 도넛 대신 순위 가로 바로 — 홈 'EC2 인스턴스 유형'(명목 카테고리의 다수 근접값 — 조각 각도보다 정확한 순위 비교, 슬라이스 상한 없음)과 Bedrock '모델별 비용'(형제 호출 수 바와 한 블록으로 비교 가능). 진짜 part-to-whole인 도넛(카테고리·파드 상태·서비스 타입·심각도·작업 상태)은 의도적으로 유지. diverging 색상쌍의 색각 안전성 검증 완료(CVD ΔE 라이트 13.9/다크 12.2, 목표 8 이상), 모든 바에 서명된 값 라벨 표시.
