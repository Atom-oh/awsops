...
variable "graph_querygen_enabled" {
  type        = bool
  description = "Hybrid LLM fallback for the registry-driven graph-sources design (2026-07-08): when a ClickHouse datasource's schema doesn't match graph_catalog.py's standard OTel-exporter shape, ask Bedrock (Haiku) to generate a candidate query, validated by a static read-only check, a best-effort AgentCore Code Interpreter sandbox check (skipped if no interpreter is provisioned), and a live LIMIT-1 dry run — only a query that survives all checks is cached (provenance='generated'). Requires datasource_diagnosis_enabled (runs inside the same datasource_index job). bedrock:InvokeModel is already granted via worker_lambda_diagnosis; this flag adds only the GRAPH_QUERYGEN_ENABLED env + the Code Interpreter session IAM (requires agentcore_enabled). false (default) = 0 resources/IAM, $0, the catalog's deterministic 'unavailable' row stands. SCOPE AS OF 2026-08-04: this same flag now also gates the diag-signal fallback (signal_catalog_gen.py) for EVERY fallback-eligible datasource kind, not just the ClickHouse graph query — so an environment that already has it on gains Bedrock generation + connector dry runs for those kinds on deploy alone. Two call sites, one flag, deliberately: both are the same 'deterministic catalog missed, ask the LLM for one query, validate it before caching' decision, and splitting them would ask an operator to consent twice to the same risk. Review MAJOR (4 models across 3 lenses) — the description previously named only the graph-query call site."
  default     = false
  validation {
