...
| **GATED** | FinOps 기본 권장 엔진(결정론적 룰 → `finops_findings`, LLM은 설명만) | `finops_baseline_enabled` | OFF | 일별 Fargate 배치, `/cost`는 Aurora만 읽음(요청 경로 라이브 AWS 호출 0). requires `workers_enabled`만(CE/Compute Optimizer/COH 권한은 워커 롤에 직접 부여 — `agentcore_enabled` 무관). read-only — AWS 리소스 변경 경로 없음(ADR-005 무관) | ADR-019 |
...
| [019](019-finops-baseline-recommendations.md) | FinOps 기본 권장 엔진 (ADR-012 확장) | 결정론적 룰 엔진이 판정·금액·우선순위를 소유, LLM은 국소 설명만(숫자 불일치 시 폐기); 금액 근거는 CE+`inventory_resources`+Compute Optimizer/COH로 한정(CUR 부재 — CUR 전제 룰은 `requires_cur`로 미구현 등록); 절감액 미상 시 NULL(0 금지); 일별 배치, 요청 경로 라이브 AWS 호출 없음; `finops_baseline_enabled`(기본 false). 부수: ADR-012가 서술했던 COH IAM 권한을 이 PR에서 실제로 부여 | 비용최적화 |
