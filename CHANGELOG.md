...
- Full-fleet aggregates past the 500-row cap: capped inventory pages now fetch server-side GROUP BYs (state tiles, both distribution donuts, the state-filter and facet dropdown options, and the exact total) over the WHOLE scoped fleet — v1 ran its summary/status/type-distribution SQL fleet-wide, and the v2 sample-based counts were silently inaccurate above 500 resources. One aggregation call replaces the previous true-total summary call; on failure everything degrades to the sample and the donut titles now disclose it with the sample qualifier (previously unlabeled). The table, Top-N bars, and highlight cards deliberately stay sample-based (already qualified).
...
- 500행 캡 초과 시 전수 집계: 캡에 도달한 인벤토리 페이지가 서버 측 GROUP BY(상태 타일, 분포 도넛 2종, 상태 필터·패싯 드롭다운 옵션, 정확한 총계)를 전체 스코프 플릿 기준으로 조회 — v1은 집계 SQL을 전 플릿에 실행했고 v2의 표본 기반 수치는 500대 초과 시 조용히 부정확했음. 집계 호출 1회가 기존 총계용 summary 호출을 대체하며, 실패 시 표본으로 폴백하고 도넛 제목에 '(표본 기준)'을 공지(종전엔 무표기). 테이블·Top-N 바·하이라이트 카드는 의도적으로 표본 유지(기존 표기 그대로).
