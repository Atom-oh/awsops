...
/** Shared eligibility for canvas search, selection and graph traversal, before display caps. */
...
export function selectE2eGraph(graph: E2eGraph, selection: E2eSelection): E2eView {
  ...
  let selected = validFocus
    ? reachable([validFocus], eligible) : eligible;
  ...
  if (!validFocus && !query) {
    // Inventory arrives first, but must not exhaust the display budget before observations.
    // Interleave network neighborhoods and services, keeping each connection with its endpoints
    // and identity bridges before unrelated inventory. Do not traverse through shared constructs.
    ...
    const connections = filtered.nodes.filter(node => node.kind === 'connection' && node.layer === 'network');
    ...
    for (let i = 0; i < Math.max(connections.length, services.length); i++) {
      if (connections[i]) {
        ...
        for (const node of [id, ...endpoints, ...identities,
          ...neighbors(identities, ['service', 'configuration']), ...neighbors([id], ['context'])]) {
          ordered.add(node);
        }
      }
      if (services[i]) {
  ...
  const nodes = [...ordered].slice(0, bound(selection.maxNodes, 350)).map(id => byId.get(id)!);
  const visibleIds = new Set(nodes.map(node => node.id));
  // Dense configuration edges must not hide the observations whose nodes we just reserved.
