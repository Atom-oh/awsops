...
- **READ_WRITE capability** + the **`action_catalog` / `write_action_refs` / executor** plumbing + the **mutating gate dependency (ADR-029/036)** — ADR-029/036 were REVERSED (substrate frozen). **→ 2026-06-14: ADR-040 is the scoping decision** — **external knowledge/comms writes (Slack/Notion/Confluence/Jira/ServiceNow, NON-AWS-resource) are un-frozen** under ADR-040's 7 hard conditions (esp. exfiltration DLP/redaction + destination allowlist; §7 mutating-gate controls; flag-OFF). **AWS-resource mutation + autonomy stay permanently reversed.** ADR-039 P3 (the external-comms write slice) is therefore **governed by ADR-040**, not blanket-blocked; the AWS-resource write slice remains frozen.
...
- P2 implementation (Integrations management/resolution layer): commits `8cd21b7`…`5641b0e` (migration + integrations CRUD + SSRF registration guard + `/api/integrations` + resolver/chat wiring + per-space UI; web vitest 598 green).
- P2-infra implementation: **inc1** (commits `3846f34`,`381b5c3`) edge ingress carve-out (applied). **inc2** egress live MCP connection — `agent.py` runtime Secrets-Manager fetch by `credentials_ref` ARN + connection-time SSRF (https/DNS-recheck/private-opt-in/redirect-manual) + api_key/bearer/sigv4 transports + per-integration failure isolation; resolver/agentcore/chat carry the connection details; `integrations_enabled`-gated dedicated KMS + scoped Secrets/KMS IAM (apply via `-target`; `agent.py` via `make agentcore`). web vitest 605 green + agent unittest 21 green.
