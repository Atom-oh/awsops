...
beforeEach(() => {
  vi.resetModules();
  vi.clearAllMocks();
  vi.stubEnv('SSM_RUNTIME_ARN_PARAM', undefined);
...
describe('AgentCore status runtime parameter', () => {
  ...
  it.each([
    { parameter: undefined, expected: '/ops/awsops-v2/agentcore/runtime_arn' },
    { parameter: '/ops/fixture/agentcore/runtime_arn', expected: '/ops/fixture/agentcore/runtime_arn' },
  ])('preserves the configured or legacy path: $parameter', async ({ parameter, expected }) => {
    vi.stubEnv('SSM_RUNTIME_ARN_PARAM', parameter);
    const { getAgentCoreStatus } = await import('./agentcore-status');
    expect((await getAgentCoreStatus(true)).runtime?.id).toBe('awsops_v2_agent-fixture');
    expect(mocks.ssm).toHaveBeenCalledTimes(1);
    expect(mocks.ssm.mock.calls[0][0].input.Name).toBe(expected);
  });
});
