...
describe('selectE2eGraph — filtering before bounds', () => {
  it('keeps observed connections, services and their identity neighborhood ahead of unrelated inventory', () => {
    ...
    // A dense configuration graph must not consume the edge budget either.
  ...
  it('retains complete flow neighborhoods when observations themselves exceed the node cap', () => {
    const graph = buildE2eGraph(input({
      services: services(),
      network: [observation(Array.from({ length: 200 }, () => flow({
        local: endpoint({ podName: 'web-1', podNamespace: 'shop' }),
      })))],
    }));
    const view = selectE2eGraph(graph, {});
    expect(view.edges.some(e => e.evidence === 'service')).toBe(true);
    expect(view.edges.some(e => e.evidence === 'identity')).toBe(true);
    const first = view.nodes.find(n => n.kind === 'connection')!;
    expect(view.edges.filter(e => e.evidence === 'network' && (e.source === first.id || e.target === first.id))).toHaveLength(2);
    expectNoDanglingEdges(view);
  });

  it('ignores a focus excluded by evidence filters rather than hiding eligible data', () => {
    const graph = buildE2eGraph(input({ configured: configured(), services: services() }));
    const focusId = graph.nodes.find(n => n.layer === 'service')!.id;
