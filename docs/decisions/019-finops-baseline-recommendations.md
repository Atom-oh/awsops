...
- **금액 근거는 Cost Explorer + `inventory_resources` + Compute Optimizer/Cost Optimization Hub/Budgets로 한정.** CUR을 전제로 하는 룰(태그 커버리지 지출 상세, Bedrock 토큰 라인아이템 등)은 룰 카탈로그에 `status: requires_cur`로만 등록하고 실행하지 않는다 — 산출 불가를 조용히 감추지 않는다.
...
- **일별 배치 적재, 요청 경로에서 라이브 AWS 호출 없음.** `finops_baseline_enabled` 플래그(기본 false, `workers_enabled`만 요구)로 게이팅된 Fargate 워커 job이 하루 한 번 CE/Compute Optimizer/COH를 호출해 `finops_findings`에 적재하고, `/cost`의 새 섹션은 Aurora만 읽는다. 이 job의 CE/Compute Optimizer/COH/Budgets 권한은 워커 task·Lambda 롤에 **직접** 부여되며(`agentcore_enabled` 게이팅의 agent Lambda `CostRead` Sid와 무관, 별도 최소 권한) `agentcore_enabled`를 요구하지 않는다. Cost Explorer 호출 횟수는 `finops_runs.ce_api_calls`로 계측한다.
- **오탐 가드는 항목을 숨기지 않고 `needs_review`로 강등한다.** DR 대기, 컴플라이언스 보존, Graviton/Spot 부적합, 관측 기간 부족, 태그 커버리지 낮음 — 가드 히트는 `guard_hits`에 기록되고 사용자가 확인할 수 있다.
...
- Cost Explorer 호출은 건당 과금이므로($0.01/건), 룰 수가 늘수록 배치 비용이 증가한다 — `ce_api_calls` 계측으로 상시 감시한다.
